...
MODE="${1:-full}"
MODEL_PATH="${E8_TOPR_DENSE_MODEL_PATH:-/root/models/Qwen2.5-0.5B-Instruct}"
WORK_DIR="${E8_TOPR_DENSE_WORK_DIR:-/root/experiment_output/e8_joint_fitted_reference_beta_topr_dense_001}"
BANK="${E8_TOPR_DENSE_BANK:-/root/experiment_output/e8_oracle_bank_v2/data/offline_bank_v2.jsonl}"
VAL="${E8_TOPR_DENSE_VAL:-/root/experiment_output/e8_oracle_bank_v2/data/val.jsonl}"
BASE_CONFIG="${E8_TOPR_DENSE_BASE_CONFIG:-configs/countdown_e8_base_rl_replay_0p5b.yaml}"
GRID_CONFIG="configs/countdown_e8_oracle_offline_v2_joint_fitted_reference_beta_topr_dense_0p5b.yaml"
EXPECTED_COMMIT="${E8_TOPR_DENSE_EXPECTED_COMMIT:-}"
...
if [[ -n "${EXPECTED_COMMIT}" ]] && [[ "$(git rev-parse HEAD)" != "${EXPECTED_COMMIT}" ]]; then
  echo "checkout does not match E8_TOPR_DENSE_EXPECTED_COMMIT" >&2
  exit 2
fi
...
expected_betas = (0.0, 0.01, 0.02, 0.04, 0.08, 0.125, 0.25, 0.5)
assert scan.EXPERIMENT_ID == (
...
print("dense beta-TOPR fixed-profile preflight: PASS")
PY
}
...
if float(summary.get("topr_beta", -1.0)) != 0.25:
    raise SystemExit("liveness checkpoint is not the frozen beta=0.25 anchor")
