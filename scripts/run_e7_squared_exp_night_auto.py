...
from drpo.e7_squared_exp_night_runtime_autotune import select_runtime
...
def _validate_existing_run_identity(work_dir: Path, selected_workers: int) -> None:
    ...
    if existing != selected_workers:
        raise RuntimeResourceError(
            "existing night-suite identity fixes max_workers="
...
def _write_gae_gate(work_dir: Path) -> Path:
    payload = {
        ...
        "gae_lambda": 0.95,
        "reason": (
            "verified ordered-trajectory and terminal/truncation contract "
            "unavailable"
        ),
        "branches_started": 0,
        ...
        json.dumps(
            {
                ...
                "selection_scope": (
                    "representative KL-PPO branch under a CPU/RAM safety ceiling"
                ),
            },
            sort_keys=True,
        ),
