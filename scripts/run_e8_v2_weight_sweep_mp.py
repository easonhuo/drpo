...
import countdown_e8_oracle_offline_v2_matrix as mx
...
def build_cells() -> list[mx.Cell]:
    ...
    for mult in MULTIPLIERS:
        for seed_off in SEED_OFFSETS:
            ...
            name = f"base_{method}_{mult_str}_seed{seed_off}" if mult > 0 else f"base_positive_only_seed{seed_off}"
            c = mx.Cell(
                name=name,
                init="base",
                method=method,
...
def run_cell_process(cell: mx.Cell, gpu: str, result_queue: Queue):
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        result_queue.put((cell.name, 1, str(e)))
...
def main() -> int:
    ...
    cells = build_cells()
    print(f"[sweep] {len(cells)} cells, running in PARALLEL on GPUs {GPUS}", flush=True)

    # Distribute cells to GPUs round-robin
    gpu_assignments = {}
    for i, cell in enumerate(cells):
        gpu = GPUS[i % len(GPUS)]
        gpu_assignments[cell.name] = gpu

    results = {}

    # Launch cells in batches of len(GPUS)
    result_queue = Queue()
    processes = []

    for i, cell in enumerate(cells):
        gpu = gpu_assignments[cell.name]
        p = Process(target=run_cell_process, args=(cell, gpu, result_queue))
        p.start()
        processes.append((cell.name, p))
        print(f"[sweep] started {cell.name} on GPU {gpu}", flush=True)

        # Limit concurrent processes to len(GPUS)
        if len(processes) >= len(GPUS):
            # Wait for one to finish
            name, rc, payload = result_queue.get()
            results[name] = {"returncode": rc, "status": "OK" if rc == 0 else "FAIL", "payload": payload}
            print(f"[sweep] finished {name} -> rc={rc}", flush=True)
            # Join the process
            for n, proc in processes:
                if n == name:
                    proc.join()
                    processes.remove((n, proc))
                    break
    ...
    manifest = {
        ...
        "cells": {k: {"returncode": v["returncode"], "status": v["status"]} for k, v in results.items()},
        "all_ok": all(r["returncode"] == 0 for r in results.values()),
    }
