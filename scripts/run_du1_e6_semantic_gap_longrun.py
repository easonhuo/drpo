#!/usr/bin/env python3
"""Launch the minimum-change D-U1 E6 semantic-gap formal long-run."""
...
DEFAULT_CONFIG = "configs/du1_e6_semantic_gap_longrun.yaml"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the frozen minimum-change E6 semantic-gap protocol"
    )
...
def main() -> int:
    ...
    if git_text(repo, "status", "--porcelain"):
        raise SystemExit("Formal E6 semantic-gap execution requires a clean worktree")
    ...
    runner = repo / "src" / "drpo" / "du1_e6_semantic_gap_longrun.py"
    command = [
        sys.executable,
        str(guard),
        "--experiment-id",
        ...
        "run_manifest.json",
        "--source-file",
        "scripts/run_du1_e6_semantic_gap_longrun.py",
        "--source-file",
        "src/drpo/du1_e6_semantic_gap_longrun.py",
        "--source-file",
        "src/drpo/du1_e6_semantic_gap.py",
        "--source-file",
    ...
    if result.returncode == 0:
        ...
        print(
            f"Formal semantic-gap run exited with code {result.returncode}; failure "
            f"evidence was preserved in {work} and packaging was attempted at {artifact}.",
            file=sys.stderr,
        )
