#!/usr/bin/env python3
"""Autotuned launcher for the E8 alpha=1 c-only scan development pilot."""
...
from drpo import countdown_e8_alpha1_c_scan_common as common
...
ADAPTER_ID = "e8_alpha1_c_scan_cuda_dev_v1"
...
def _selection(args: argparse.Namespace, config: dict[str, Any]) -> dict[str, Any]:
    ...
    fingerprint = {
        ...
        "tuned_runtime_field": "active_gpu_devices_with_fixed_slots",
        "max_processes_per_device": int(execution["parallel_cells_per_gpu"]),
    ...
    document = selection_document(
        ...
        limitations=[
            "fixed_two_processes_per_gpu_after_prior_measured_pilot",
            "configured_vram_floor_precedes_actual_two_step_liveness",
            "smoke_probes_one_worker_before_full_two_slot_run",
...
def _core_command(
    ...
    result = [
        sys.executable,
        str(repo / "src" / "drpo" / "countdown_e8_alpha1_c_scan_runtime.py"),
        command,
        "--model_path",
    ...
    if command == "run":
        result.extend(
            [
                ...
                str(
                    common.load_yaml(args.grid_config)["execution"][
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        config["registration_state"] == "dev_code_first_unregistered"
        and not args.allow_dev_unregistered
    ):
        raise RuntimeResourceError(
            "--allow-dev-unregistered is required for this code-first pilot"
        )
