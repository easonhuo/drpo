...
WORK_DIR="${E7_W0_HIGHC_LIVENESS_WORK_DIR:-outputs/e7/w0_highc_actor_liveness_001}"
PROBE_STEPS="${E7_W0_HIGHC_LIVENESS_PROBE_STEPS:-500}"
PROBE_SECONDS="${E7_W0_HIGHC_LIVENESS_PROBE_SECONDS:-120}"
MAX_WORKERS="${E7_W0_HIGHC_LIVENESS_MAX_WORKERS:-2}"
...
python scripts/run_e7_w0_highc_actor_auto.py plan \
  --repo-root "${REPO_ROOT}" \
  --contract "${CONTRACT}" \
  --run-spec "${RUN_SPEC}" \
  --grid "${GRID}" \
