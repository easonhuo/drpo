...
PREPARED_ROOT="${E7_SQEXP_GAE_PREPARED_ROOT:-outputs/e7/sqexp_gae_002/prepared}"
WORK_DIR="${E7_SQEXP_GAE_WORK_DIR:-outputs/e7/sqexp_gae_003}"
MAX_WORKERS="${E7_SQEXP_GAE_MAX_WORKERS:-40}"
export E7_SQEXP_GAE_PREPARED_ROOT="${PREPARED_ROOT}"

if [[ -n "$(git status --porcelain)" ]]; then
  echo "refusing to run from a dirty checkout" >&2
  exit 2
fi
for required in "${CONTRACT}" "${RUN_SPEC}" "${GRID}" "${PREPARED_ROOT}"; do
