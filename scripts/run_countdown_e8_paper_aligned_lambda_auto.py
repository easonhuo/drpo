#!/usr/bin/env python3
"""Resource-aware launcher for paper-aligned Countdown E8 lambda round 1."""
...
from drpo.runtime_autotune import (
    GIB,
...
ADAPTER_ID = "e8_paper_aligned_lambda_round1_cuda_v1"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("command", choices=("plan", "calibrate", "smoke", "run"))
    parser.add_argument("--model_path", required=True)
    parser.add_argument("--work_dir", required=True)
    parser.add_argument("--bank", required=True)
    parser.add_argument("--val", required=True)
    parser.add_argument("--base_config", default=common.DEFAULT_BASE_CONFIG)
    parser.add_argument("--grid_config", default=common.DEFAULT_GRID_CONFIG)
...
def _reject_existing_nonauto_work_dir(work_dir: Path) -> None:
    ...
    evidence = (
        ...
        work_dir / "TAPER_CALIBRATION.json",
    )
...
def _selection(args: argparse.Namespace, config: dict[str, Any]) -> dict[str, Any]:
    ...
    usable_host_bytes = int(machine.effective_memory_available_bytes * (1.0 - host_headroom))
    ...
    default_gpu_slots = int(execution["default_gpu_slots"])
    if default_gpu_slots < 1:
        raise RuntimeResourceError("default_gpu_slots must be positive")
    requested_slots = default_gpu_slots if args.max_devices is None else args.max_devices
    if requested_slots < 1:
        raise RuntimeResourceError("--max-devices must be positive")
    effective_max = min(host_slot_limit, requested_slots)
    ...
    fingerprint = {
        ...
        "configured_default_gpu_slots": default_gpu_slots,
        "requested_gpu_slots": requested_slots,
        "max_devices_override": args.max_devices,
...
def _core_command(args: argparse.Namespace, command: str, *, selected_ids: list[str]) -> list[str]:
    repo = Path(__file__).resolve().parents[1]
    result = [
        sys.executable,
        str(repo / "src" / "drpo" / "countdown_e8_paper_aligned_lambda_runtime.py"),
        command,
        "--model_path",
...
def main(argv: list[str] | None = None) -> int:
        ...
        json.dumps(
            {
                ...
                "fixed_alpha": common.FIXED_ALPHA,
                "lambda_values": list(common.LAMBDA_VALUES),
    ...
    if args.command == "calibrate":
        ...
        return plan_code or _run_core(args, "calibrate", selected_ids)
    if args.command == "smoke":
        for command in ("plan", "calibrate", "smoke"):
            code = _run_core(args, command, selected_ids)
            if code != 0:
                return code
        return 0
    ...
    for command in ("plan", "calibrate", "smoke"):
        ...
        if code != 0:
            return code
    gate = json.loads((work_dir / "SMOKE_GATE.json").read_text(encoding="utf-8"))
