#!/usr/bin/env python3
"""Run the 84-branch E7 high-c A2C/PPO pilot with measured CPU resources."""
...
from drpo.e7_w0_highc_runtime_autotune import revalidate_runtime, select_runtime
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--probe-seed", type=int, default=992_001)
...
def _reject_legacy_work_dir(work_dir: Path) -> None:
    ...
    if any(path.exists() for path in existing) and not selection.is_file():
        raise RuntimeResourceError(
            "refusing to attach the high-c auto runner to a pre-existing fixed work directory"
        )
...
def _validate_existing_run_identity(
    ...
    if existing != selected_workers or existing_workers != selected_workers:
        raise RuntimeResourceError(
            "existing high-c run identity fixes a different worker count"
        )
    if existing_digest != selection_digest:
        raise RuntimeResourceError(
            "existing high-c run identity is not bound to the immutable selection"
        )
