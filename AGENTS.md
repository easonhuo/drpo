...
The following controls are mandatory for formal runs under `GOV-EXP-ARTIFACT-02` and `GOV-EXP-ARTIFACT-03`:
...
* The merge-equivalent checkout must be a real Git checkout/fetch or a verified Git bundle/source capsule that contains the expected commit object and complete tree. A plain GitHub source archive may support static review, but by itself is not a commit-bound checkout and cannot satisfy the formal-run or final pre-delivery gate. Never reconstruct a base tree from web snippets, parsed pages, copied files, or a hand-built repository. If the exact Git base cannot be acquired, do not label the package verified and do not deliver it as ready for blind application.
