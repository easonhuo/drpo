...
def build_shared_start(
    ...
    environment_audit["passed"] = bool(environment_audit["passed"] and initial_geometry["passed"])
    if not environment_audit["passed"]:
        raise RuntimeError(
            f"initial D-U1 environment audit failed for seed {seed}: {environment_audit}"
        )

    optimizer = make_optimizer(model, protocol)
    initial_gap = float(
        (2.0 * model.rarity_coordinate(environment.train["states"]).abs()).mean().detach()
    )
    ...
    for step in range(1, protocol.positive_warm_start_steps + 1):
        ...
        loss = (
            ...
            + protocol.rarity_logit_anchor_coefficient * rarity_logit_anchor_loss(model, states)
        )
    ...
    warm_summary = {
        ...
        "parameter_delta_norm": float((parameter_vector(model) - initial_vector).norm()),
    ...
    environment_audit["passed"] = bool(environment_audit["passed"] and warm_geometry["passed"])
    if not environment_audit["passed"]:
        raise RuntimeError(
            f"post-warm-start D-U1 audit failed for seed {seed}: {environment_audit}"
        )
...
def terminal_classification(
    ...
    first_rows = [
        ...
        if terminal.window_1_steps[0] <= int(row["step"]) <= terminal.window_1_steps[1]
    ]
    second_rows = [
        row
        for row in trajectory
        if terminal.window_2_steps[0] <= int(row["step"]) <= terminal.window_2_steps[1]
    ]
    ...
    for metric, tolerance in terminal.metric_tolerances:
        first = float(np.mean([float(row[metric]) for row in first_rows]))
        second = float(np.mean([float(row[metric]) for row in second_rows]))
...
def run_method(
    *,
    ...
    def record(step: int, numerical_failure: bool = False) -> None:
        ...
        utility_valid = (
            metrics["utility_oracle_sign_valid_fraction"] >= protocol.utility_sign_fraction_min
        )
        rarity_valid = (
            metrics["rarity_coordinate_positive_fraction"] >= protocol.utility_sign_fraction_min
        )
        environment_valid = bool(utility_valid and rarity_valid)
        environment_failure = environment_failure or not environment_valid
        prototype_event = (
            metrics["prototype_effective_support"] < protocol.prototype_effective_support_boundary
        )
        rarity_event = (
            min(
    ...
    for step in range(1, protocol.maximum_steps + 1):
        ...
        negative_loss, last_diagnostics = negative_loss_and_diagnostics(
            cells=cells,
            spec=spec,
            calibration=calibration,
            protocol=protocol,
        ...
        last_diagnostics["rarity_logit_anchor_loss"] = float(anchor.detach())
        loss = (
            positive_loss
            + protocol.negative_alpha * negative_loss
            + protocol.rarity_logit_anchor_coefficient * anchor
        )
        measure = step % protocol.evaluation_interval_steps == 0 or step == protocol.maximum_steps
    ...
    summary = {
        ...
        "terminal_formal_acceptance": bool(terminal_result["formal_acceptance"]),
        ...
        "minimum_utility_oracle_sign_valid_fraction": min(
            float(row["utility_oracle_sign_valid_fraction"]) for row in trajectory
        ),
        ...
        "max_stepwise_budget_match_error": max(
            float(row["stepwise_budget_match_error"]) for row in trajectory
        ),
