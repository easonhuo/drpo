...
def replace_functions(source: str, replacements: dict[str, str]) -> str:
    ...
    for name, replacement in replacements.items():
        ...
        spans.append((start, end, name, replacement.rstrip() + "\n"))
    for start, end, name, replacement in sorted(spans, reverse=True):
        current = "".join(lines[start - 1 : end])
        if not current.startswith(f"def {name}("):
            raise RuntimeError(f"Unexpected source boundary for {name}")
...
if "# Recovery/runtime lifecycle ownership" in runtime:
    raise RuntimeError("Recovery/runtime split already applied")

replacements = {
    "_read_json_object": '''def _read_json_object(path: Path) -> dict[str, Any]:
    return e8_runtime.read_json_object(path)
''',
    "_successful_attempt_matches_current_identity": '''def _successful_attempt_matches_current_identity(
...
''',
    "_effective_recovery_config": '''def _effective_recovery_config(
    config: Mapping[str, Any],
    output_root: Path,
) -> dict[str, Any]:
...
''',
    "_reusable_cell_manifests": '''def _reusable_cell_manifests(
    config: Mapping[str, Any],
    output_root: Path,
...
''',
    "_recovery_stage_plan": '''def _recovery_stage_plan(
    config: Mapping[str, Any],
    output_root: Path,
    *,
    base_model_path: str,
...
''',
    "_hardlink_file": '''def _hardlink_file(source: Path, destination: Path) -> None:
    e8_runtime.hardlink_file(source, destination)
''',
    "_replace_path_prefix": '''def _replace_path_prefix(value: Any, source: str, destination: str) -> Any:
    return e8_runtime.replace_path_prefix(value, source, destination)
''',
    "_recovery_checkpoint_snapshot": '''def _recovery_checkpoint_snapshot(
    config: Mapping[str, Any],
    output_root: Path,
    snapshot_root: Path,
    *,
...
''',
    "cmd_compact_logs": '''def cmd_compact_logs(config: Mapping[str, Any], output_root: Path) -> dict[str, Any]:
...
main = replace_functions(main, replacements)
for stale_import in ("import tarfile\n", "import zipfile\n"):
    main = main.replace(stale_import, "", 1)
...
runtime = runtime.rstrip() + runtime_addition + "\n"

MAIN.write_text(main, encoding="utf-8")
RUNTIME.write_text(runtime, encoding="utf-8")

# Fail fast if the dependency direction was accidentally inverted.
if "e8_multitask_exp_tuning" in RUNTIME.read_text(encoding="utf-8"):
    raise RuntimeError("Runtime module must not import or name the composition root")
