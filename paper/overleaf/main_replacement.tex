...
remoteness-to-influence feedback loop. Under finite-order growth of the
unweighted influence, the resulting exponential envelope makes the
weighted far-field contribution vanish asymptotically. Viewed
distributionally, this smooth reweighting extends DRPO's optimistic
selection principle from hard selection to continuous learner-relative
control.
...
\begin{itemize}
    ...
    self-amplifying far-field tail.
...
\subsection{Minimum-Order Control of Remoteness-Driven Expansion}
\label{subsec:min_order_control}

We first ask how fast a negative-branch attenuation coefficient must
decay to control the far-field expansion identified in
Section~\ref{sec:theory}. Let \(r=d_{\mathrm{Mah}}(a,\mu)\) denote the
standardized distance in the Gaussian mean geometry. The theory gives
\[
D_\mu(a)-C_\Sigma=\frac12 r^2,
\qquad
R_\mu(a)=\|\nabla_\mu\log\pi_\mu(a)\|_2^2\asymp r^2 .
\]
Thus, the raw far-field response is quadratic in standardized distance,
and after attenuation its residual scale is
\(\omega(r)R_\mu(a)\). The minimum-order control requirement is the
following.

\paragraph{Proposition 5.1. Minimum matching order.}
Suppose that the raw far-field response satisfies
\(I(r)=\Theta(r^2)\), and let
\(I_\omega(r)=\omega(r)I(r)\) be the residual response after applying a
nonnegative attenuation coefficient \(\omega(r)\). Then
\[
    I_\omega(r)=O(1)
    ...
    \qquad (r\to\infty).
\]
Hence reciprocal-quadratic attenuation in standardized distance is the
minimum matching order required to make the quadratic far-field response
bounded. Any weaker attenuation cannot generally control the
remoteness-driven expansion. The proof is given in
...
Section~\ref{subsec:min_order_control} gives a minimum-order condition
for bounded far-field control. However, boundedness alone does not answer
whether a remote historical negative update should continue to exert a
nonzero residual force. We do not assume that negative feedback is
intrinsically harmful. Rather, motivated by signed advantage updates in
policy optimization, replay-discrepancy control, and gradient-alignment
criteria for learning-signal usefulness
\citep{novati2019remember,fakoor2020p3o,du2018adapting}, we introduce a
learner-relative notion of negative-update utility.

Operationally, for a negative sample \(z=(s,a)\), a weight
\(w\in[0,1]\), and the raw repulsive update
\(g_\theta^-(z)=-\widehat A^-(z)\nabla_\theta\log\pi_\theta(a\mid s)\),
we define its empirical utility on an evaluation batch \(\mathcal B\) as
\begin{equation}
...
\end{equation}
This quantity measures the task effect of applying the weighted negative
update under the current learner. We use it to state the design
assumption underlying DRPO.

\paragraph{Design principle 5.2. Distance-dependent utility.}
The utility of a negative update is learner-relative and
distance-dependent: nearby negative feedback may provide useful local
boundary or correction information, whereas its utility should decrease
as the sample becomes increasingly remote from the current policy. In
the far field, once a historical negative update no longer provides
useful local information, it should not retain a persistent nonzero
repulsive force.

This principle strengthens the bounded-control floor in
Section~\ref{subsec:min_order_control}. Let \(D_\theta(s,a)\) denote
learner-relative remoteness, and let \(I(D)\) denote the raw far-field
influence scale expressed in the same remoteness coordinate. A merely
bounded residual response can still assign a nonzero force to an
unhelpful far-field sample. We therefore require the negative-branch
attenuation \(\omega(D)\) to satisfy
\begin{equation}
    \omega(D) I(D)
    \longrightarrow 0
    \qquad
...
\end{equation}
Thus, DRPO should preserve nearby negative feedback while driving
far-field residual influence to zero. Appendix~\ref{app:negative_update_utility}
formalizes this utility view and gives sufficient conditions under which
the utility-optimal far-field effective step vanishes.
...
\paragraph{Proposition 5.3. Vanishing finite-order far-field influence.}
...
in both domains. In Hopper, the implemented actor-gradient diagnostic increases with standardized distance while matched absolute advantage remains approximately flat.
 In Countdown, the implemented trainable-gradient diagnostic increases with mean-token surprisal while the negative coefficient remains fixed.
increases with mean-token surprisal, while the negative coefficient
remains fixed. These external diagnostics indicate that remoteness-correlated negative update influence appears in realistic neural actors. The controlled experiments below then isolate the policy-score source and test causal transmission.
...
The external summary in Figure~\ref{fig:taper_control_and_transfer} shows that DRPO preserves the benefit of
near-field negative feedback while avoiding the excessive displacement
caused by insufficient far-field suppression. It achieves higher
normalized return in Hopper and higher verifier success in Countdown
than positive-only learning and global negative scaling, while producing
fewer support or variance-boundary events. The controlled
stability--utility principle therefore transfers to both continuous
offline control and autoregressive policy optimization.
...
Assume \(I(r)=\Theta(r^2)\). Then there exist constants
\(0<c_1\le c_2<\infty\) and \(r_0>0\) such that, for all \(r\ge r_0\),
\[
    c_1 r^2
    \le
    I(r)
...
\[
    \omega(r)I(r)\le M
    \qquad
    \text{for all sufficiently large } r .
\]
Using \(I(r)\ge c_1 r^2\), we obtain
\[
    \omega(r)
    \le
    \frac{M}{c_1 r^2}
...
\[
    \omega(r)\le C r^{-2}
\]
for all sufficiently large \(r\). Using \(I(r)\le c_2 r^2\),
\[
    I_\omega(r)
    =
    \omega(r)I(r)
    ...
    O(1).
\]
Therefore reciprocal-quadratic attenuation is the minimum matching order
needed to make the quadratic far-field response bounded.
