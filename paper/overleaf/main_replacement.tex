...
We begin by testing whether negative-gradient amplification increases
with learner-relative remoteness in realistic policy-optimization
tasks. We study Hopper and Countdown because they cover two
substantially different policy families: a state-conditioned Gaussian
policy for continuous control and an autoregressive token policy for
mathematical generation.

In Hopper, we restrict the analysis to transitions with negative
learned-critic advantages and control for absolute advantage magnitude.
We then order these transitions by their standardized distance from the
current Gaussian policy and measure the norm of the resulting
per-sample actor gradient. In Countdown, we analogously compare
unsuccessful responses with the same verifier outcome, using
current-policy mean token surprisal as learner-relative remoteness.

Figure~\ref{fig:external_far_field} shows that the negative
parameter-gradient norm increases systematically with remoteness in
both tasks. Thus, negative samples with comparable quality exert
increasingly strong optimization influence as they move farther from
the current learner. The agreement between continuous control and
autoregressive generation provides external evidence that far-field
amplification is not specific to one action space or policy
parameterization. The controlled experiments that follow isolate its
source and test whether it causally propagates into policy drift and
collapse.
...
In the continuous source-isolation protocol, C-U1 places negative
actions on the same reward contour, giving them identical fixed
advantages but different standardized distances from the current
Gaussian policy. D-U1 constructs the categorical counterpart by holding
negative labels fixed while varying their current-policy probabilities.
As shown in Figure~\ref{fig:controlled_source}, Gaussian score and full
parameter-gradient influence increase sharply with remoteness. For
categorical policies, the direct-logit score remains bounded, but
negative suppression persists even after the selected action becomes
rare, continually increasing its logit gap and pushing its probability
toward the simplex boundary. Thus, remoteness produces abnormal negative
influence independently of sample quality, although its manifestation
differs between continuous and categorical policies. This experiment
identifies the source of the influence, not its downstream causal effect.

\subsubsection{Causal Transmission into Drift and Collapse}
\label{sec:controlled_transmission}

We next evolve a nonlinear state-conditioned Gaussian policy with
learnable mean and variance, together with the corresponding categorical
support dynamics in D-U1. All intervention branches share the data,
fixed labels, initialization, optimizer, and training horizon. They
differ only in whether near- or far-field negative updates are retained,
removed, or capped; globally scaled controls match the resulting
negative-update budget without using remoteness.

Figure~\ref{fig:controlled_transmission} shows that removing near-field
negative updates leaves the harmful dynamics largely intact, whereas
removing or limiting the far-field component substantially reduces
policy displacement and prevents the associated variance or
probability-support boundary events. Abnormal far-field influence also
precedes the observed drift, rather than emerging only after performance
has deteriorated. These interventions therefore identify far-field
negative influence, rather than negative feedback in general, as a
causal transmission path to policy instability. Task-performance
collapse, support or variance-boundary events, and NaN/Inf numerical
...
The causal results show why the far-field tail must be controlled, but
not why all negative feedback should be removed. Positive-only learning
...
We compare positive-only learning, uncontrolled signed updates, global
negative scaling, hard filtering, reciprocal tapering, and exponential
tapering while changing only the negative weighting rule. Two matched
comparisons are used. Near-field-retention matching preserves comparable
local negative influence and isolates differences in far-field decay,
whereas aggregate-budget matching tests whether a selective taper
...
provides the strongest stability--utility trade-off across C-U1 and
D-U1. Positive-only learning avoids far-field instability but remains at
the positive-support ceiling. Uncontrolled updates initially benefit
from local repulsion before remote pressure induces excessive drift and
boundary events. Global scaling suppresses useful near- and harmful
far-field feedback indiscriminately, while hard filtering introduces an
abrupt cutoff. The exponential taper instead retains near-field
repulsion while rapidly suppressing the far-field tail, yielding higher
task performance and held-out-context generalization with weaker policy
drift than the matched alternatives.
...
Figure~\ref{fig:external_tapers} shows that DRPO preserves the benefit of
near-field negative feedback while avoiding the excessive displacement
caused by uncontrolled signed updates. It achieves higher normalized
return in Hopper and higher verifier success in Countdown than
positive-only learning, global negative scaling, and the uncontrolled
baseline, while producing fewer support or variance-boundary events.
The controlled stability--utility principle therefore transfers to both
continuous offline control and autoregressive policy optimization.
...
Table~\ref{tab:d4rl_performance} compares published offline-RL
baselines with Positive-only, Global-$\alpha$, Reciprocal-Linear,
Reciprocal-Quadratic, and DRPO under our common implementation.
The shared objectives and taper functions are defined in
Appendix~\ref{app:shared_methods}; D4RL-specific score provenance,
uncertainty statistics, remoteness construction, and implementation
details are provided in Appendix~\ref{app:d4rl_methods}.
...
\toprule
&
\multicolumn{4}{c|}{Published offline-RL baselines}
&
\multicolumn{2}{c|}{Protocol controls}
&
\multicolumn{3}{c}{Remoteness-aware methods}
\\
\cmidrule(lr){2-5}
\cmidrule(lr){6-7}
\cmidrule(lr){8-10}
...
\footnotesize
\textit{Note.}
Pos.\ denotes Positive-only, G-$\alpha$ denotes Global-$\alpha$,
and Rec.-L and Rec.-Q denote Reciprocal-Linear and
Reciprocal-Quadratic. Results marked by $\dagger$ are reported by prior
work rather than protocol-matched reruns. The table reports task-level
means. DRPO results are averaged over 10 runs; the corresponding
standard deviations and seed-level results are reported in
Appendix~\ref{app:d4rl_methods}. Shared method definitions are provided
in Appendix~\ref{app:shared_methods}. A dash denotes a result pending
the formal run.
\end{minipage}
...
total normalized return of \emph{[TBD]}. It improves over Positive-only,
Global-$\alpha$, Reciprocal-Linear, and Reciprocal-Quadratic by
\emph{[TBD]}, \emph{[TBD]}, \emph{[TBD]}, and \emph{[TBD]},
respectively. Its advantage is largest on the medium-replay tasks,
where broader behavioral coverage creates stronger far-field pressure,
while remaining competitive on the medium and medium-expert datasets.
These results indicate that selective exponential attenuation preserves
useful negative feedback more consistently than either removing it,
scaling it globally, or applying slower reciprocal decay.
...
validation split, and test puzzles. We include
AsymRE~\citep{arnal2025asymmetric} and
TOPR~\citep{leroux2025tapered} as published off-policy baselines,
together with protocol controls and alternative remoteness-aware
tapers. The shared objectives and taper functions are defined in
...
\toprule
&
\multicolumn{2}{c|}{Published off-policy baselines}
&
\multicolumn{2}{c|}{Protocol controls}
&
\multicolumn{3}{c}{Remoteness-aware methods}
...
DRPO achieves the highest greedy verifier success among the
protocol-matched off-policy methods, reaching \emph{[TBD]}. It improves
over AsymRE and TOPR by \emph{[TBD]} and \emph{[TBD]}, respectively,
and exceeds Positive-only and Global-$\alpha$ by \emph{[TBD]} and
\emph{[TBD]}. DRPO also obtains the highest pass@$k$ while maintaining
...
The comparisons distinguish four possible sources of improvement:
retaining only successful responses, rebalancing all negative responses
independently of remoteness, correcting generic off-policy reuse, and
selectively attenuating the learner-relative far-field tail. The results
show that neither removing negative feedback nor applying a single
response-independent adjustment fully recovers the performance of
remoteness-aware control. The reciprocal tapers further test whether the
remoteness signal alone is sufficient. DRPO provides the strongest
overall result by retaining useful near-field negative feedback while
more rapidly suppressing remote unsuccessful responses.
