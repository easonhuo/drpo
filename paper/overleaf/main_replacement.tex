...
(Proof: Appendix~\ref{app:exp_vanishing_influence}; the exponential form is
a convenient envelope, not the unique choice.)
...
Appendix~\ref{app:gaussian_drpo_boundedness} gives an explicit
...
The output-coordinate proof and finite-crossing conditions are given in
Appendices~\ref{app:categorical_self_limiting}
and~\ref{app:categorical_restoring_balance}.

DRPO's exponential taper also arises as the optimizer of a
generalized-I-divergence-constrained reweighting of the finite
negative-update measure; unlike standard DRO over normalized distributions,
this construction may reduce total negative-update mass
(Appendix~\ref{app:finite_measure_variational}).

\section{Experiments}
\label{sec:experiments}

Experiments proceed from occurrence to identification, control, and transfer.
External probes test whether the predicted far-field pattern appears in
realistic continuous-control and autoregressive policies; controlled
...
near/far structure; final task-level evaluations cover D4RL and structured
generation. We use C-U1, a Gaussian continuous-action contextual bandit, and
D-U1, an unordered categorical environment with shared representations, as
controlled environments with known ground truth for source isolation, policy
dynamics, and held-out-context generalization. External diagnostics use nine
D4RL locomotion cells~\citep{fu2020d4rl} and nine structured-generation
tasks---Countdown~\citep{hutton2002countdown,pan2025tinyzero}, seven Reasoning
Gym tasks~\citep{stojanovski2025reasoninggym}, and
WikiSQL~\citep{zhong2017seq2sql}---covering continuous-action and discrete
autoregressive policies. Controlled source-isolation reports policy-score
response, whereas neural probes report implemented actor-gradient magnitude
to test whether the near-to-far pattern persists in trainable updates.
Task-performance collapse, support/variance-boundary events, and NaN/Inf
failures are reported separately. Full configurations and protocols are in
Appendices~\ref{app:experimental_details} and~\ref{app:controlled_protocols}.
...
SG-9 averages Countdown with the eight transfer tasks; the Global-$\alpha$
entry is pending its protocol-matched transfer-task sweep.}
...
standardized remoteness from the current Gaussian policy and structured-generation
responses by mean-token surprisal. The SG-9 row aggregates Countdown with the
eight transfer tasks. Its Reciprocal-Linear, Reciprocal-Quadratic, and DRPO
entries summarize the registered finite-horizon response curves; the
Global-$\alpha$ aggregate is left unreported until the corresponding
transfer-task sweep is run. Because external tasks lack counterfactual
near/far geometry, Table~\ref{tab:taper_transfer_summary} provides
descriptive transfer evidence rather than a closed cross-task ranking.
