...
\textbf{(a)} Matched operating points (left: near-field-retention matched;
right: matched for \(B(\omega)\), the weighted sum of per-sample
output-coordinate update magnitudes). Bars split \(B(\omega)\) into near-
and far-field contributions; dashed traces show held-out-context reward.
\textbf{(b)} Countdown-0.5B fixed-budget transfer reports the best observed
two-seed late-window Pass@8 from each registered development sweep.
}
...
them indiscriminately. In the fixed-budget Countdown transfer summary, the
best observed point in the two-seed development sweep is DRPO's 15.7\%
late-window Pass@8, followed by Reciprocal-Quadratic (14.5\%), Positive-only
(14.0\%), Reciprocal-Linear (13.6\%), and Global-\(\alpha\) (14.2\%).
...
lack counterfactual near/far geometry, Figure~\ref{fig:taper_control_and_transfer}b
