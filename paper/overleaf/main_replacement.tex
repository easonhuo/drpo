...
\]
For categorical logits \(z\), with \(p=\operatorname{softmax}(z)\),
\[
D_z(a)
=
\log\sum_j e^{z_j}-z_a,
...
\nabla_z^2D_z(a)
=
\operatorname{diag}(p)-pp^\top\succeq0,
\]
because
...
Proposition~\ref{prop:distance_strength} gives constants
\(0<c_1\le c_2<\infty\) and \(D_0\) such that, for all \(D\ge D_0\),
\[
c_1D
\le
R(D)
...
For a corresponding negative sample \(z\), write its raw update as
\(g(D)=g_\theta^-(z)\) with \(\|g(D)\|_2=I(D)\), and define
\[
...
\(L_{\mathcal B}\)-Lipschitz around \(\theta\), then for the corresponding
negative sample \(z\) and all sufficiently large \(D\),
\[
\left|
\widehat{\mathcal U}_{\mathcal B}
\bigl(z;\omega_{\mathrm{Exp}}(D)\mid\theta\bigr)
\right|
\le
L_{\mathcal B}
\|\Delta\theta_{\mathrm{Exp}}(D)\|_2
