...
hopper-medium
& 58.5 & \underline{59.3} & \textbf{66.3} & 57.3 \\
...
hopper-replay
& \textbf{95.0} & 60.9 & \underline{94.7} & 87.8 \\
...
hopper-expert
& \textbf{105.4} & \underline{98.0} & 91.5 & \underline{98.0} \\
...
\small
\textit{Note.} ``replay'' and ``expert'' abbreviate ``medium-replay'' and
...
Registered model-independent generators construct frozen
prompt-indexed banks of verified oracle solutions and verifier-incorrect
completions. Within each task, fixed labels and update budgets are shared by
...
DPO~\citep{rafailov2023direct}. The Countdown DPO entry reuses the
registered short-SFT predecessor run, whereas the eight transfer tasks use
the frozen baseline-matrix protocol with fresh-LoRA initialization from the
base model and an exact frozen copy as the DPO reference. The shared
objectives, baseline definitions, and taper functions are given in
...
\small
\textit{Note.}
For Countdown, AsymRE, \(\beta\)-TOPR, and DRPO initialize their
trainable LoRA adapters directly from the same Qwen2.5-0.5B-Instruct backbone;
\(^{\ddagger}\)the reported DPO entry reuses the registered short-SFT
predecessor run. For the eight transfer tasks, the frozen baseline matrix uses
fresh-LoRA initialization from the same base model for all three baseline
families; DPO freezes an exact copy of its initial policy adapter as the
reference. The TOPR arm implements the joint fitted-reference
\(\beta\)-TOPR variant; it is not a canonical frozen-behavior reproduction of
\citet{leroux2025tapered}.
\end{minipage}

\end{table}

In the currently populated Countdown row, DRPO has the highest observed
Pass@8 among the compared methods. The eight transfer-task baseline entries
remain unreported until the registered 176-cell matrix completes its terminal
audit.
...
The eight transfer tasks use 5,000 training prompts and 500 validation
prompts under the unified paper-facing protocol. Countdown reuses its
predecessor corpus with 6,000 training puzzles and 500 validation puzzles.
Reserved test data are not used for coefficient tuning. The transfer-task
interface selects 16 verifier-incorrect responses per training prompt;
Countdown retains the predecessor bank's unique-negative consumer. Replay data
remain fixed during optimization, while learner-relative mean completion-token
surprisal is recomputed from the current policy. This zero-refresh construction
isolates stale-negative reuse without storing learner-relative remoteness in the
bank.
...
candidate budgets and checkpoint-selection rule.
...
the jointly fitted reference adapter during training. The completed Countdown
...
a formal method ranking. The eight transfer-task scans belong to the
registered formal baseline matrix and are not treated as results before its
terminal audit.

\paragraph{DPO.}

Canonical DPO~\citep{rafailov2023direct} is trained with the same LoRA
parameterization as the other structured-generation methods. For Countdown,
the reported predecessor run initializes both the policy and frozen reference
from the registered short-SFT checkpoint; this row-specific difference is
marked by \(^{\ddagger}\) in
Table~\ref{tab:structured_generation_performance}. For the eight transfer
tasks, the frozen baseline matrix uses fresh LoRA from the base model and
copies the initial policy adapter exactly into the frozen reference before the
first DPO update. For each prompt \(x\), the verified oracle solution
...
Before policy optimization, registered model-independent generators
construct frozen prompt-indexed replay banks. For each of the eight transfer
tasks, every training prompt has one verifier-confirmed oracle completion and
16 model-independent verifier-incorrect completions under the registered
training interface. Countdown reuses its predecessor bank, whose paper-facing
consumer deduplicates exact expressions and may expose 9--16 unique negatives
per prompt; a fixed-width 16-slot representation does not create additional
unique negatives. The paper-facing trainers canonicalize exact outputs before
consumption. The banks store no learner-relative near/far label and no
behavior-policy log probability. Verifier outcome, response length,
output-validity status, and task-specific error metadata remain separate stored
...
The eight transfer tasks use 5,000 training prompts and 500 validation
prompts per task, while Countdown reuses its predecessor protocol with 6,000
training puzzles and 500 validation puzzles. Reserved test data are not used
for coefficient tuning. All structured-generation runs use 1,200 optimizer
updates and no early stopping; the transfer-task matrix selects 16 negatives
per training prompt, while Countdown retains its predecessor unique-negative
consumer. Pass@8 is the primary sampled-decoding metric. The common LoRA
configuration is rank 32, alpha 64, and dropout 0.05.
...
For each of the eight transfer tasks, AsymRE scans
...
selection. Countdown is reused from the registered predecessor comparison and
has no new cells in this baseline matrix.
