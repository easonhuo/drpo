...
whether that displacement improves task utility is an empirical question. In off-policy settings, however,
...
sizes. Holding \(\mathbf m_+\) and \(\mathbf m_-\) fixed, the algebraic
signed-target displacement from \(\mathbf m_+\) grows without bound as
\(\rho\to1^-\), unless \(\mathbf m_+=\mathbf m_-\). A finite
equilibrium exists only while the signed target remains in the attainable
mean-parameter space.

\textbf{Critical regime \((\rho=1)\).}
If \(\mathbf m_+\neq\mathbf m_-\), the policy-output coordinate has a
constant nonzero drift and admits no finite equilibrium. For the Gaussian
and categorical coordinates considered here, this drift persists for all
forward time. If \(\mathbf m_+=\mathbf m_-\), every point is stationary,
but no point is an isolated asymptotically stable equilibrium.
...
Assume \(0<p<q\), so that both positive and negative aggregate
feedback masses are nonzero, with negative feedback dominant.
...
one exists, is unique and repelling. Every nonstationary initialization
(and every initialization if no finite stationary point exists) yields
continuous dynamics and discrete trajectories with fixed step size
\(\alpha>0\) that eventually leave every compact subset of the logit
space. Along some subsequence, the corresponding probability
...
held-out-context generalization. The nine D4RL locomotion dataset cells
\citep{fu2020d4rl} and structured-generation tasks provide continuous-action
and discrete autoregressive diagnostics, respectively. The
structured-generation tasks comprise
Countdown~\citep{hutton2002countdown,pan2025tinyzero},
seven Reasoning Gym tasks~\citep{stojanovski2025reasoninggym}, and
WikiSQL~\citep{zhong2017seq2sql}. Full configurations are in
...
In the C-U1 intervention protocol, all branches share the
...
a complete policy-optimization method on D4RL locomotion and
structured-generation tasks.
...
report normalized return for D4RL and verifier success for
structured-generation tasks,
...
We evaluate the nine D4RL Gym locomotion dataset cells formed by HalfCheetah,
...
baselines---CQL \citep{kumar2020conservative}, TD3+BC
\citep{fujimoto2021minimalist}, and IQL \citep{kostrikov2021offline}---%
under the D4RL evaluation protocol \citep{fu2020d4rl,todorov2012mujoco}. The shared objectives and
taper functions are defined in Appendix~\ref{app:shared_methods};
D4RL-specific score provenance, remoteness
...
We evaluate Qwen2.5-0.5B-Instruct~\citep{qwen2024qwen25} on the
structured-generation tasks listed in
Section~\ref{sec:environments_datasets}. Task success is determined by
the corresponding deterministic verifier, and Pass@8 is the primary task
metric.

A registered model-independent generator constructs frozen
prompt-indexed banks of verified oracle solutions and verifier-incorrect
completions. Fixed labels and update budgets are shared by
protocol-matched methods, while learner-relative statistics are recomputed
during training. This zero-refresh limit isolates stale-negative reuse
(Appendices~\ref{app:structured_generation_dataset}
and~\ref{app:structured_generation_methods}).

We compare DRPO with
AsymRE~\citep{arnal2025asymmetric},
the Joint Fitted-Reference \(\beta\)-TOPR
variant~\citep{leroux2025tapered}, and canonical
DPO~\citep{rafailov2023direct}, preceded by a short SFT stage to ensure
a task-capable initialization. The shared objectives, baseline definitions,
...
Assume \(K\ge2\) below and let \(p=\pi_z(a)=e^{-D_z(a)}\). Since
\[
...
(1-p)^2+\sum_{j\ne a}\pi_z(j)^2.
\]
The non-selected probabilities are nonnegative and sum to \(1-p\), so
\[
\frac{(1-p)^2}{K-1}
\le
\sum_{j\ne a}\pi_z(j)^2
...
For the Gaussian and reference-logit categorical coordinates considered
here, \(\mathcal H\) is the full coordinate space, so the drift persists
for all forward time. More generally, the same formula holds for as long
as the trajectory remains in \(\mathcal H\). If
...
\begin{proof}
Let \(r=q-p>0\). Because \(0<p<q\), the normalized positive and
negative moments used below are well defined.
...
It remains to prove escape for every nonstationary initialization. Since
\(g(z)=b+r\pi_z\) is bounded, the continuous flow
\(\dot z=g(z)\) exists for all forward time. Let
\[
c_0=\|g(z_0)\|_2^2>0.
\]
Along the flow,
...
v(D)=\frac{g(D)}{I(D)}.
\]
Assume \(J\) is twice continuously differentiable on the local update
segments considered below. Suppose the local update space admits an
...
is locally harmed along those directions. If far-field growth remains
