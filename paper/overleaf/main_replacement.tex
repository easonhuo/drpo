...
\end{equation}
Both exhibit linear drift and admit no finite equilibrium; the drift persists
for all forward time whenever the natural-parameter domain is full or forward
invariant along the drift direction.
...
\end{equation}
Because Lemma~\ref{lem:far_field_utility} is local to
\(\rho\in[0,\rho_0]\), let
\[
w_{\mathcal U,\mathrm{loc}}^\star(D)
\in
\arg\max_{\substack{0\le w\le1\\ \eta w I(D)\le\rho_0}}
\mathcal V_D\!\left(\eta w I(D)\right).
\]
By the lemma's quadratic bound and feasibility of zero,
\begin{equation}
0\le \eta I(D)w_{\mathcal U,\mathrm{loc}}^\star(D)
\le \frac{2[a(D)]_+}{\kappa}
\longrightarrow0.
\label{eq:oracle_weighted_influence_vanishes}
\end{equation}
For fixed \(\eta>0\), the weight also vanishes if
\(\liminf_{D\to\infty}I(D)>0\). Thus, when the raw far-field response
remains nonvanishing or grows, the utility-optimal local sample weight itself
must shrink so that the weighted response vanishes.
