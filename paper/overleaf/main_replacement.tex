...
Policy-based methods learn through signed feedback: positive updates
increase the likelihood of actions estimated to be relatively
preferable, whereas negative updates reduce the probability of actions
estimated to be relatively undesirable. Prior work has shown that
negative feedback can suppress undesirable behavior and support
generalization beyond positive-only learning
\citep{zhu2025negative,song2025good}. This makes the two signals
complementary: positive updates provide attraction toward relatively
preferable actions, while negative updates encode boundaries and
exclusion information absent from positive updates. In many off-policy
settings, historical samples are reused across multiple policy updates.
The same negative sample may therefore be revisited after earlier
updates have already reduced its probability under the current policy.
This raises our central question: how does repeated reuse change the
influence of useful negative feedback, and when can that influence
become excessive repulsion?

Against this background, we identify a self-amplifying far-field
dynamic in repeated off-policy updates. A negative update reduces the
likelihood of a historical action, increasing its learner-relative
distance or surprisal. Because the policy score---and therefore the
update influence---depends on the current policy geometry, the resulting
remoteness changes the next update. Repeated reuse can therefore cause
the action's negative influence to grow disproportionately or decay too
slowly as the learner moves farther away. We identify learner-relative
remoteness as the state variable linking one negative update to the
next. The precise amplification law depends on the policy family:
continuous Gaussian policies can exhibit distance-dependent score
growth, whereas categorical direct-logit scores remain bounded even as
repeated suppression drives action probabilities toward the simplex
boundary. Thus, initially useful local negative feedback may evolve into
excessive far-field repulsion: the danger lies not only in where a
negative sample starts, but in how its influence changes under repeated
reuse. Quantitatively, uncontrolled Gaussian remoteness can grow
geometrically under repeated negative reuse, while categorical
one-vs-rest log-odds fall linearly; exponential tapering reduces both
far-field laws to logarithmic growth in magnitude.

Existing methods already mitigate off-policy instability through
filtering, importance weighting, probability- or surprisal-aware gating,
and other forms of tapered negative updates
\citep{novati2019remember,fakoor2020p3o,peng2019advantage,
schulman2017proximal,fujimoto2019off,kumar2020conservative,
arnal2025asymmetric,leroux2025tapered}. What remains unresolved is
whether learner-relative remoteness itself amplifies negative
influence. Because remoteness is naturally confounded with sample
quality and negative-advantage magnitude, we use matched controls that
hold these factors fixed while varying only distance or rarity. These
controls isolate remoteness as an independent amplifier, while targeted
near--far interventions test whether its far-field influence propagates
into policy drift, support- or variance-boundary events, and
task-performance degradation.

Having identified the remoteness-dependent amplification of individual
negative updates, we next ask how these forces aggregate with positive
attraction to determine the global policy dynamics. We answer this
question through \emph{Repulsive Dynamics}, an aggregate theory of
signed policy updates. Our analysis shows that moderate repulsion can
move the policy beyond the Positive-only solution while retaining a
finite stable equilibrium. As negative influence approaches the
strength of positive attraction, the restoring force weakens and
persistent drift can emerge. Once repulsion dominates, no finite stable
equilibrium remains. Together, these regimes define a transition in
policy geometry and stability; whether a stable displacement improves
task utility remains an empirical question.

The theory therefore identifies the appropriate control target: not
negative feedback as a whole, but its excessive far-field contribution.
We therefore propose DRPO, a learner-relative nonlinear control of
negative updates. DRPO measures distance or surprisal under the evolving
policy and applies a nonlinear taper in the same remoteness coordinate
that drives the identified mechanism. It preserves informative
near-field negative updates while progressively attenuating increasingly
remote contributions, thereby interrupting the
remoteness-to-influence feedback loop. The taper is exactly one below
its threshold, vanishes in the far field, and suppresses every finite
polynomial order of unweighted influence. Viewed distributionally, this
is a finite-measure optimistic reweighting analogue. The geometric
criterion cannot by itself identify rare far-field samples that remain
semantically aligned with true task improvement; attenuating such
samples can discard useful signal.

Our empirical study follows a layered evidence chain. We first examine
whether far-field amplification appears in realistic continuous-control
and autoregressive policy-learning tasks. We then use controlled
continuous and categorical environments to isolate learner-relative
remoteness from correlated factors and test its causal transmission into
policy dynamics. Strength and taper experiments further test the
predicted transition from useful negative feedback to stable
extrapolation, drift, and instability under selective, global, and
budget-matched controls. Finally, we return to external tasks to test
whether remoteness-aware control improves task-level outcomes.
Throughout, task-performance collapse, support- or variance-boundary
events, and NaN/Inf numerical failure are reported as distinct outcomes.

Our contributions are summarized as follows:
\begin{itemize}
    \item We formulate repeated off-policy negative updates as
    learner-relative repulsive dynamics and develop an aggregate theory
    that characterizes stable extrapolation, persistent drift, and the
    loss of a finite stable equilibrium.

    \item We identify learner-relative remoteness as an independent
    amplifier of negative influence and use targeted near--far
    interventions to trace its transmission into policy drift,
    boundary events, and task-performance degradation.

    \item We propose DRPO, a learner-relative nonlinear attenuation of
    negative updates that acts in the same remoteness coordinate,
    preserving informative local feedback while suppressing the
    self-amplifying far-field tail. We distinguish selective tapering
    from global scaling by its far-field order and prove Gaussian
    ultimate boundedness and categorical self-limiting suppression.

    \item We evaluate the resulting theory and method through a layered
    evidence chain combining controlled source identification, causal
    intervention, and external continuous-control and
...
are evaluated under an evolving current policy; Section~4 studies the
actor-side dynamics induced by this reuse.

All aggregate masses below use this same update base measure. We write
\(p=\mathbb E_\nu[\widehat A^+]\),
\(q=\mathbb E_\nu[\widehat A^-]\), and \(\rho=q/p\) when \(p>0\).
Thus a conditional sample coefficient is not multiplied again by an
empirical sampling frequency already contained in \(\nu\).
...
For sequence policies we distinguish token surprisal, total trajectory
surprisal \(-\log\pi_\theta(y\mid x)\), and mean-token surprisal
\(-|y|^{-1}\log\pi_\theta(y\mid x)\). Total and mean surprisal differ by
a positive constant at fixed length, but variable-length normalization
need not preserve their global ordering. This distinction is used when
relating Countdown's length-normalized coordinate to TOPR.
...
split used below. In Gaussian policies, learner-relative remoteness
...
score remains bounded. The next result shows how historical reuse turns
this static distance--response relation into a dynamic feedback process.
...
amplification for categorical logits. These per-sample dynamics do not by
themselves imply global divergence, because other historical samples,
especially positive samples, exert competing forces. We therefore next
analyze their aggregate balance.
...
Appendix~\ref{app:aggregate_equilibrium}. Let \(p\) and \(q\) denote the
aggregate positive and negative update masses, combining sample
frequency, advantage magnitude, and any fixed sample weighting, and
write \(\rho=q/p\) when \(p>0\). Let \(\mathbf m_+\) denote the
positive-only target, the mean-parameter point selected by positive
feedback alone. When \(q>0\), let \(\mathbf m_-\) denote the
negative-feedback moment, the magnitude-weighted center of samples
carrying negative feedback.
...
Theorem~\ref{thm:policy_family_manifestations} shows that the same
negative-dominant instability has different policy-family
manifestations. In fixed-covariance Gaussian policies, mean runaway
increases the separation from every fixed historical action and produces
unbounded mean-score responses. Categorical policies instead approach
the probability-simplex boundary while their per-sample logit scores
remain bounded. Fixed covariance is sufficient for the Gaussian result;
learned covariance is not required. Neither policy-family conclusion
alone implies task-performance collapse or a NaN/Inf numerical failure.
...
These results are mechanism limits for frozen data, fixed advantages,
and unbounded reuse. Data refresh, actor--critic coupling, and reference
regularization change the field. For example, adding the
KL-geometry spring \(-\gamma\Sigma^{-1}(\mu-\mu_0)\) to the unweighted
Gaussian field yields the following complementary restoration result.
...
Section~\ref{sec:theory} shows that negative feedback should not be
controlled uniformly: nearby negative samples may contain useful local
correction signals, whereas increasingly remote historical negatives can
produce persistent or amplified repulsion. DRPO therefore controls only
the negative branch with a learner-relative remoteness taper, aiming to
retain local negative feedback while suppressing far-field influence.
This section derives the minimum control order, motivates a stronger
vanishing-tail requirement, and instantiates it with a smooth exponential
taper.

\subsection{Loop-Gain Order and Far-Field Utility}
...
Bounded loop gain alone does not determine whether a remote update is
useful. We retain the original empirical definition of negative-update
utility and its sufficient shrinkage result in
Appendix~\ref{app:negative_update_utility}, but use it only as motivation:
...
We now choose a concrete taper that satisfies the vanishing-residual
requirement from Section~\ref{subsec:distance_dependent_utility}. Since
the far-field responses considered in Section~\ref{sec:theory} have
finite growth order, a sufficient choice is an exponential envelope over
...
remoteness scale, and \(\lambda>0\) controls the decay rate. This taper
leaves negative updates unchanged when \(D\leq\tau\), and attenuates them
smoothly when \(D>\tau\).

\begin{proposition}[Vanishing finite-order far-field influence]
\label{prop:exp_vanishing_influence}
Let \(I(D)\) denote the raw far-field influence scale. If, for some
...
This result provides the asymptotic guarantee required by
Section~\ref{subsec:distance_dependent_utility}: DRPO preserves nearby
negative feedback while driving sufficiently remote residual influence to
zero. The exponential form is not unique; it is a simple smooth envelope
that dominates finite-order growth. The proof is given in
...
tails. Global scaling fails L0, reciprocal-linear tapering is critical at
...
Before embedding the taper in the full actor update, we record its
output-coordinate consequences once positive attraction is included.
...
that equilibrium. Finally,
\(\rho_{\mathrm{eff}}(\mu)=p^{-1}\sum_iq_i\omega_i(D_i)\to0\) along
every escaping sequence; this is a diagnostic consequence, not the
boundedness proof.
\end{theorem}

For \(a=\lambda/c\) and
\(\widetilde\tau_i=\tau_i-C_\Sigma\), the proof uses
\(B_\omega\le\sqrt{\lambda_{\max}(\Sigma^{-1})}\sum_iq_iG_i\), where
\[
G_i=\begin{cases}
a^{-1/2}e^{a\widetilde\tau_i-1/2},&\widetilde\tau_i<0,\\
\sqrt{2\widetilde\tau_i},&\widetilde\tau_i\ge0,\ a^{-1/2}\le\sqrt{2\widetilde\tau_i},\\
a^{-1/2}e^{a\widetilde\tau_i-1/2},&\widetilde\tau_i\ge0,\ a^{-1/2}>\sqrt{2\widetilde\tau_i}.
\end{cases}
\]
The separate first branch is required because a negative shifted
threshold activates the taper at radius zero. The full proof is in
...
\subsection{DRPO Update and Policy-Family Instantiations}
\label{subsec:drpo_update}

We now embed the exponential remoteness taper into the signed actor
update. Using the learner-relative remoteness
\(D_\theta(s,a)=-\log\pi_\theta(a\mid s)\), define
\(\bar D_\theta(s,a)=\operatorname{sg}[D_\theta(s,a)]\), where
\(\operatorname{sg}[\cdot]\) denotes stop-gradient. DRPO keeps the
positive branch unchanged and attenuates only the negative branch:
\begin{equation}
...
\end{equation}
The remoteness statistic is recomputed under the current policy but
detached before being used as a sample coefficient. Thus, DRPO is a
learner-relative weighting of historical negative feedback rather than a
differentiable distance regularizer.

In the exponential-family field of
Theorem~\ref{thm:aggregate_equilibria}, DRPO changes the same aggregate
object: define
\(q_\omega=\mathbb E[\widehat A^-\omega]\) and
\(M_{-,\omega}=\mathbb E[\widehat A^-\omega T(a)]\). When
\(0<q_\omega<p\), the controlled signed target is
\(m_\omega^*=(pm_+-M_{-,\omega})/(p-q_\omega)\); when
\(q_\omega=0\), use \(M_{-,\omega}=0\) directly without defining a
normalized negative moment. The critical and negative-dominant cases
remain those of Theorem~\ref{thm:aggregate_equilibria}.

\paragraph{Gaussian policies.}
For a Gaussian policy with mean \(\mu_\theta(s)\) and fixed covariance
\(\Sigma\), define the squared standardized distance
\begin{equation}
\begin{aligned}
    D_{\mathrm G,\theta}(s,a)
    &=
    d_{\mathrm{Mah}}^2(a,\mu_\theta(s)) \\
    &=
    (a-\mu_\theta(s))^\top
    \Sigma^{-1}
    (a-\mu_\theta(s)).
...
\end{equation}
Since \(D_\theta(s,a)-C_\Sigma=\frac12D_{\mathrm G,\theta}(s,a)\),
absorbing the additive constant and the factor \(1/2\) into the
threshold and scale gives the Gaussian DRPO weight
\begin{equation}
    \omega_{\mathrm G}(s,a)
    =
    \exp
    \left\{
        ...
        \left[
            \frac{
                D_{\mathrm G,\theta}(s,a)-\tau_{\mathrm G}
            }{
                c_{\mathrm G}
            }
        \right]_{+}
...
For a categorical policy, learner-relative remoteness is the surprisal of
the selected action, \(D_\theta(s,a)=-\log\pi_\theta(a\mid s)\).
Substituting this into Equation~\eqref{eq:drpo_exp_weight} gives
\begin{equation}
    \omega_{\mathrm C}(s,a)
    =
    \min
...
\end{equation}
Thus, the same DRPO update becomes squared-standardized-distance control
for Gaussian policies and surprisal control for categorical policies.

\subsection{Variational and TOPR Interpretations}
...
\]
otherwise the corresponding full \(G_i\) branch above is used. Replacing
the weighted sum by \(q\) and \(\mu_i\) by \(\mu_{\min}>0\) is only a
finite-support worst-case relaxation.

\subsection{DRPO in the Space of Negative-Update Controls}
\label{subsec:negative_update_controls}

The DRPO update in Equation~\eqref{eq:drpo_actor_field} places negative
feedback controls into a common form: positive updates keep unit weight,
while the negative branch is controlled by a coefficient \(\omega(D)\).
This framing connects to advantage-weighted and positive-filtered policy
learning, as well as tapered or probability-aware off-policy updates
\citep{peng2019advantage,nair2020awac,kostrikov2021offline,
grigsby2021closer,arnal2025asymmetric,leroux2025tapered}.
The key distinction here is whether this coefficient depends on
learner-relative remoteness.

For sequence ablations we distinguish canonical TOPR computed from total
trajectory surprisal from the mean-token, length-normalized analogue;
they are not labeled as the same control.

Distance-agnostic controls apply the same rule to all negative samples,
whether they preserve, globally downweight, or remove the negative
branch. They are useful baselines, but they cannot separate local
negative feedback from the far-field tail: reducing one necessarily
reduces the other.

Remoteness-aware controls instead make \(\omega(D)\) decrease with
distance. Their main difference is tail behavior. Reciprocal tapers
provide polynomial decay and can yield bounded residual influence at the
matching order, whereas exponential tapering decays smoothly and
dominates finite-order far-field growth.

DRPO therefore occupies the regime targeted by the preceding analysis:
it preserves local negative feedback while removing persistent far-field
residual influence.
...
transfers to complex policies and can improve overall task performance.
...
are provided in Appendix~\ref{app:experimental_details}. We use policy-score norm for mechanism-source claims whenever the policy coordinate is controlled. In neural external diagnostics, we report implemented actor-gradient diagnostics only to check whether the policy-level remoteness effect appears in actual trainable updates.
...
fixed. With the sample-quality coefficient controlled, these diagnostics test whether learner-relative remoteness is reflected in the implemented actor updates.

Figure~\ref{fig:external_far_field} shows policy-family-specific
signatures. In Hopper, the implemented actor-gradient diagnostic
...
also contain the network Jacobian. These external diagnostics indicate
that remoteness-correlated influence appears in realistic neural actors.
The controlled experiments below isolate the policy-score source and
test causal transmission.
...
realistic tasks, but they are not designed to identify the causal
mechanism. In Hopper and Countdown, sample quality, policy-relative
remoteness, critic or verifier coefficients, data coverage, and
subsequent policy dynamics remain partially entangled. We therefore turn
to controlled environments, whose role is identification rather than
realism: they allow sample quality and learner-relative remoteness to be
varied independently, and allow near- and far-field negative updates to
be intervened on directly. We first isolate whether remoteness itself
changes negative influence and then test whether that influence causally
...
right panel is reserved for terminal-audited task-level outcomes; dashes
remain until the corresponding formal runs close.
}
...
tail. In the closed-loop models, this restores Gaussian ultimate
boundedness and changes categorical probability suppression from
exponential to polynomial decay; a suitable restoring force can then
support a finite stable categorical crossing. Stable off-policy
optimization should control reused-feedback geometry, not merely its
sign or magnitude.
...
\bibliography{example_paper.bib,missing_references.bib}
...
zero and one. Define
\(I(\kappa)=\int\phi(w_\kappa)d\xi^-\). Dominated convergence gives
\[
\lim_{\kappa\to\infty}I(\kappa)=0,
\qquad
\lim_{\kappa\downarrow0}I(\kappa)
=\xi^-(\{h>0\})=\delta_0.
\]
On a set of positive \(h\), increasing \(\kappa\) strictly increases
\(w_\kappa\) toward one and strictly decreases \(\phi(w_\kappa)\).
Thus \(I(\kappa)\) is continuous and strictly decreasing from
...
\(I(0)=|\xi^-|\), so \(w\equiv0\) becomes feasible only at the later
...
text use the registered fixed-variance branch unless explicitly labeled
otherwise; learnable-variance variants are labeled as audit ablations. A finite crossing of
...
generalization claim. Its reconstruction parameters and direct-softmax initial conditions are
maintained in the corresponding registered experiment specification.
...
The exact dataset versions, transition counts, and preprocessing statistics
follow the frozen experiment configuration.
...
Exact generation ranges, split sizes, and corpus statistics follow the
...
protocol and is described separately in
Appendix~\ref{app:countdown_methods}.
...
mean dynamics and, for learnable-variance audit runs, scale dynamics, entropy or effective support, and the first
