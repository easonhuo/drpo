...
\title[Remoteness-Aware Control of Negative Off-Policy Updates]{Breaking the Curse of Repulsion: Remoteness-Aware Control of Negative Off-Policy Updates}

% Author names, affiliations, emails, acknowledgments, and funding are
...
\keywords{Off-Policy Reinforcement Learning, Negative Feedback, Policy Optimization, Stability}
...
\end{equation}
\paragraph{Relation to existing negative-update controls.}
Positive-only sets the negative-branch weight to zero, whereas fixed
HPO/global-\(\alpha\) applies a constant weight \(\alpha\) to all
negative advantages \citep{sana2026hpo}. AsymRE changes the
positive--negative balance through the reward baseline rather than
learner-relative remoteness \citep{arnal2025asymmetric}. At the
atomic-action level, canonical TOPR's \(\min\{\pi/\mu,1\}\) is the
\(\beta=1\), behavior-threshold member of the categorical taper family
\citep{leroux2025tapered}; DRPO instead uses learner-relative threshold,
scale, and decay parameters to satisfy L0--L3. Exact identities,
extensions, and the fitted-reference experimental variant are given in
...
\begin{figure}[t]
    ...
    \Description{Two-panel external diagnostic figure. The D4RL locomotion
...
\caption{
    ...
    \Description{Two-part controlled-mechanism figure. The top heatmap factorizes coefficient magnitude and policy-score response across remoteness bins. The bottom intervention plot compares reward retention and task-collapse counts across near-field and far-field interventions.}
...
\caption{
    ...
    \Description{Phase-transition figure showing held-out-context reward and policy displacement as effective negative strength increases, with separate indicators for task collapse, boundary events, and numerical failure.}
...
The same principle is tested externally: DRPO weights Hopper transitions by
...
Countdown-0.5B fixed-budget transfer reports the best observed two-seed
late-window Pass@8 from each registered development sweep (full curves:
Figure~\ref{fig:app_countdown_taper_coefficient_response}); Hopper remains
...
shown. DRPO reports the mean over 10 runs; standard deviations and seed-level
results are in Appendix~\ref{app:d4rl_methods}.
\end{minipage}
...
raises the ratio to the \(1/L\) power, proving the
length-normalized identity above.
...
\]
and cancels that denominator on the far branch.

The TOPR arm implements the joint fitted-reference
\(\beta\)-TOPR variant; it is not a canonical frozen-behavior
reproduction of \citet{leroux2025tapered}.

The canonical population cancellation above requires the same
\(\mu\) to serve as both the outer sampling mass and the ratio
denominator. It therefore does not apply to the experimental variant,
whose outer measure is induced by prompt-uniform replay with a
per-prompt unique-negative mean and whose denominator is
\(\mu_{\mathrm{ref}}\). The experimental arm also uses
full-completion summed log probabilities rather than the
length-normalized ratio identity above.

For Gaussian atoms the
...
\Description{Controlled taper-shape comparison under near-field-retention and aggregate-budget matched operating points.}
\end{figure}
...
dense \(\beta\) scan is a provenance-limited development pilot and does
...
Joint Fitted-Reference \(\beta\)-TOPR variant instead uses the dynamic
full-completion policy/reference log-ratio defined above, not DRPO's
mean-token remoteness.
...
All protocol-matched methods initialize a fresh policy LoRA on the same
Qwen2.5-0.5B-Instruct backbone~\citep{qwen2024qwen25}. The Joint
Fitted-Reference \(\beta\)-TOPR arm additionally creates a fresh
reference LoRA with parameters copied from the initial policy adapter.
The registered methods share the AdamW
optimizer~\citep{loshchilov2019decoupled}, learning-rate schedule,
minibatch construction, number of optimizer steps, validation split,
and paired development seeds unless a method-specific protocol states
...
\begin{figure}[t]
    ...
    \Description{Single-panel Countdown coefficient-response figure comparing late-window Pass@8 for exponential, reciprocal-linear, and reciprocal-quadratic tapers against the Positive-only reference over logarithmic coefficient scales.}
