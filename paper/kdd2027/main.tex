...
negative coefficient satisfies
\(e^{-[-\log\pi+\log\mu]_+}=\min\{\pi/\mu,1\}\): it is exactly the
\(\beta=1\), sample-specific-threshold member of the taper family in
Equation~\eqref{eq:drpo_exp_weight}; length-normalizing both log
probabilities would instead yield the algebraic analogue
\(\min\{\pi/\mu,1\}^{1/L}\). The structural difference is that TOPR's
threshold is the behavior probability, whereas DRPO's threshold, scale,
and decay are learner-relative design parameters chosen to satisfy
L0--L3. The Gaussian extension under a common dominating measure,
conditional-versus-population constants, and the corresponding
boundedness bound are deferred to Appendix~\ref{app:topr_relation}.

The exact identity above characterizes canonical TOPR, whose threshold is
the behavior probability; our experimental arm is the joint fitted-reference
\(\beta\)-TOPR variant, which uses a full-completion policy/reference
log-ratio and is not the length-normalized identity above
...
\begin{figure}[t]
    ...
    \Description{Two-panel diagnostic figure. The Hopper panel compares relative actor-gradient magnitude and matched absolute advantage across standardized-distance bins. The Countdown panel compares relative actor-gradient magnitude and fixed negative coefficient across mean-token-surprisal deciles.}
...
\begin{minipage}[t]{0.28\columnwidth}
    ...
    {\scriptsize
    Hop.: Hopper return.\\
    Cnt.: Countdown late-window Pass@8.
    }
\end{minipage}
...
\textbf{(b)} Fixed-budget Countdown-0.5B transfer summary and reserved Hopper panel. Hopper entries remain open pending the formal run and terminal audit. Countdown entries report the maximum two-seed late-window Pass@8 over each method\'s registered coefficient sweep; complete coefficient-response curves are shown in Figure~\ref{fig:app_countdown_taper_coefficient_response}.
}
...
than the matched alternatives. In the fixed-budget Countdown transfer
summary, the best observed point in the two-seed development sweep is DRPO's
15.7\% late-window Pass@8,
followed by Reciprocal-Quadratic (14.5\%), Positive-only (14.0\%),
Reciprocal-Linear (13.6\%), and Global-$\alpha$ (2.4\%); the complete
coefficient-response curves are reported in Figure~\ref{fig:app_countdown_taper_coefficient_response}.
...
now reports the fixed-budget Countdown-0.5B late-window summary, while
the Hopper cells remain open pending the formal run and terminal audit.
We therefore treat the panel as partial transfer evidence rather than a
closed cross-task ranking. The controlled results establish the
...
We use a four-number variant of the Countdown arithmetic
game~\citep{hutton2002countdown}, following the verifier-guided LLM
formulation popularized by TinyZero~\citep{pan2025tinyzero}. Before
optimization, a registered model-independent generator constructs one
frozen prompt-indexed bank of verified oracle solutions and
verifier-incorrect completions; the protocol-matched methods reported
here share this frozen bank and the registered update budget. Labels
remain fixed. Method-specific learner-relative statistics, when
applicable, are recomputed during training under each registered
objective. This zero-refresh limit isolates stale-negative reuse; task
construction is in Appendix~\ref{app:countdown_dataset}, and
method-specific training protocols are in
