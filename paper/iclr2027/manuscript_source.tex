...
fixed-covariance Gaussian mean field with positive attraction and finitely many
exponentially tapered negative atoms, trajectories are ultimately bounded and
the field has at least one finite equilibrium; if an uncontrolled stable
equilibrium lies entirely inside the untapered region, the controlled and
original fields have the same local linearization. For a repeatedly suppressed
categorical action, tapering changes far-tail probability decay from exponential
to polynomial rather than imposing a nonzero probability floor. Full theorem
statements, branch conditions, and proofs are given in
Appendices~\ref{app:gaussian_drpo_boundedness},
