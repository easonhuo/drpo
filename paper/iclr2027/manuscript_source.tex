...
absent from positive-only learning \citep{zhu2025negative,song2025good}. It can displace the policy beyond the positive-only solution; whether this
improves task utility remains empirical. In off-policy settings, a historical sample may be reused after earlier updates
...
standardized distance, whereas categorical logit scores remain bounded as
selected-action probabilities approach the simplex boundary. Under isolated reuse, uncontrolled Gaussian remoteness grows
...
order. This is the curse of repulsion: a negative update makes the same historical action
...
updates unchanged, and exponentially tapers beyond the threshold. The taper dominates every finite-order
...
\textbf{(3)} matched external diagnostics and controlled interventions that
separate coefficient magnitude from policy geometry, identify a far-field
causal pathway, and test the stability--utility trade-off.
...
Both policy families satisfy the theorem's convexity assumption globally:
fixed-covariance Gaussian remoteness is quadratic in the mean, and categorical
negative log-likelihood is convex in logits
...
Aggregating positive and negative historical samples, we ask whether
competing attraction and repulsion yield a finite stable equilibrium,
persistent drift, or instability.
...
Under negative dominance, policy families differ in score and support
manifestations despite lacking a finite stable equilibrium.
...
DRPO targets the remote negative source without uniformly weakening the
negative branch.
...
to historical behavior or a reference. They classify mechanisms, not algorithms; methods may instantiate multiple
mechanisms. DRPO acts at the sample-influence level, attenuating negative feedback by
current-learner remoteness rather than a behavior-policy ratio. Appendix~\ref{app:topr_relation} gives the full
...
\(R(D)=\|\nabla D\|_2^2\), not single-step parameter displacement; we call it
the reuse \emph{loop gain}.
...
It ranks guarantees, not task performance.
...
generation. C-U1 (a Gaussian continuous-action contextual bandit) and D-U1 (unordered
categorical actions with shared representations) provide controlled
environments with known ground truth for source isolation, policy dynamics, and
held-out-context generalization. External diagnostics use nine D4RL locomotion
...
In D4RL and structured generation, learner-relative remoteness varies at fixed
sample-quality magnitude; Appendices~\ref{app:d4rl_gradient_diagnostic}
...
(Figure~\ref{fig:app_external_gradient_panels}). The approximately linear D4RL
trend agrees with the Gaussian prediction; the fixed-coefficient structured-generation
trend is consistent with persistent categorical suppression. These probes show the near-to-far pattern in realistic neural policies;
controlled experiments separate remoteness from coefficient magnitude and test
whether far-field updates drive instability.
...
External diagnostics entangle sample quality, remoteness, critic/verifier
coefficients, coverage, and downstream dynamics, so controlled experiments
independently vary coefficient magnitude and remoteness and intervene on near-
versus far-field negative updates
(Appendix~\ref{app:controlled_protocols}). C-U1 negative actions share a reward contour and fixed advantage but differ in
...
DRPO weights D4RL transitions by current-policy standardized remoteness and
structured-generation responses by mean-token surprisal; protocol-matched
...
On the D4RL offline-RL suite~\citep{fu2020d4rl},
...
remoteness taper. This is consistent with far-field source control
(Section~\ref{sec:selective_tapering_bridge}) but is task-level transfer, not a
...
Across the eight transfer tasks, reported DRPO scores exceed those of AsymRE
on five tasks and joint fitted-reference $\beta$-TOPR on seven.
...
learner-relative weighting applies when negatives are already remote under the
current policy, including replay and stale-policy settings; remoteness-aware
control is not restricted to static offline datasets.
