...
For each structured-generation task, learner-relative remoteness is measured
by current-policy mean completion-token surprisal,
\[
D_{\mathrm{SG}}(x,y)
=
-\frac{1}{T}
...
\]
where only completion tokens are included and prompt and padding positions are
excluded. At the frozen diagnostic checkpoint for each task, we evaluate
verifier-incorrect responses without applying an optimizer update.

\paragraph{Fixed-coefficient gradient measurement.}
Each probed negative response uses the same unit absolute negative coefficient.
We compute its full parameter-gradient norm, so the negative-signal magnitude
is fixed while learner-relative surprisal varies.
...
partitioned into ten equal-count bins. The mean implemented actor-gradient norm
in each bin is normalized by the lowest-surprisal bin of the same task. The
task-level normalized curves are aggregated with equal task weight, while the
corresponding task-level curves are reported separately in
...
The D4RL experiments use HalfCheetah, Hopper, and Walker2d under
...
defined under the same convention and kept fixed across all compared methods.
