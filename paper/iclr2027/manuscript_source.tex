...
Existing controls for historical-data reuse include importance weighting,
conservative or behavior-regularized objectives, and reference-regularized
...
Existing controls span advantage-weighted policy fitting, which downweights
...
We organize off-policy controls by four intervention questions: which data
are used, what learning signal they carry, how much each sample contributes,
and how the policy is anchored to historical behavior or a reference.
These questions classify control mechanisms rather than algorithms, so a
method may instantiate more than one mechanism. DRPO acts at the
sample-influence level: it attenuates negative feedback according to
remoteness under the current learner, rather than a behavior-policy ratio.
The full taxonomy and method comparisons are given in
