# DRPO / SNA2C 远场负梯度动力学研究主文档 v13（正式执行版）
...
- C-U1 正式 E1-E4：**用户已确认，门禁解除；按 v13 冻结协议执行。**
...
5. **策略：** 共享两层 MLP，state-conditioned 2D Gaussian mean 与标量 log-standard-deviation head；不使用人为方差 clamp。非有限值或 `|log_sigma|>12` 记为数值崩溃，而不是裁剪成平台。
...
10. **收敛与终态：** 每 100 steps 评估；稳定候选需在至少 10 个连续窗口中核心量归一化斜率低于 `1e-4`、更新 norm 低于 `1e-3`，随后继续训练到首次判定步数的 2 倍并要求状态分类和方法排序不反转。持续漂移则报告斜率、reward 失效时间和数值状态；最大步数按各 protocol 配置记录，不用固定步数冒充稳态。
...
| E4 | 稳定外推与泛化 | 受控负梯度能否突破 positive-only 上限，远场是否反转为有害 | 是 | 策略越过 a_plus、接近 a_star；ID/OOD reward；强度扫描；控制恢复；固定/可学习方差 |
...
| E3 | Product/Collapse 环境短程 20-seed 因果结果 | **C-U1 正式长期 20-seed 已完成**：固定方差 Baseline/Near-zero 任务崩溃或持续低 reward 漂移，Far-zero/Far-cap/Global 0/20 崩溃；可学习方差 Baseline/Near-zero 20/20 在任务阈值前发生精度坍缩，远场控制 0/20 | 正式因果实验完成；任务崩溃与数值崩溃分支分别报告 |
...
1. 冻结 C-U1 环境设计参数与 E1-E4 protocol 映射；
2. 将旧 Product 与 Extrapolation protocol 接到同一个 C-U1 数据对象，先做单 seed 回归；
3. E1 不变量与瞬时梯度重跑；
4. E2 完整饱和动力学；
5. E3 长期 Near/Far 因果干预；
6. E4 稳定外推与控制；
...
## 11.4 E3：C-U1 长期 Near/Far 因果干预（2026-06-24）

**状态：正式 20-seed 因果实验完成。** 为避免开发 seed 泄漏，正式使用 held-out seeds 30–49。固定方差与可学习方差分成两个互补分支，严格区分“任务效果崩溃但数值仍有限”和“任务阈值前先发生精度/支持坍缩”。
...
## 11.4 E3 正式协议冻结补充：均值任务崩溃与可学习方差提前失稳
...
- E3 fixed variance：因果成熟度高；主文核心结果候选。必须恢复精确任务失效阈值、动态 near/far occupancy 和阈值敏感性。
- E3 learnable variance：机制证据中高；可入附录或主文补充，但须补完整时间轨迹、sigma-floor 敏感性，并改用“支持收缩边界/无联合稳态”术语。
