...
The implementation reuses existing Python files only. The full run remains explicitly gated and is not launched by this branch. Temporary export and patch-transport scaffolding was used only to materialize the exact after-image and is absent from the final stacked diff and every scientific provenance identity.
