...
1. **Repulsive Dynamics Theory**：向量—矩阵形式刻画 finite interior equilibrium、boundary approach 与 no finite equilibrium/divergence，保留 transition matrix 和 spectral-radius analysis。
2. **Mechanism Identification**：严格解耦 advantage magnitude 与 distance/probability，并通过 near/far、common/rare interventions 形成因果证据。
3. **DRPO**：以 exponential distance/surprisal weighting 保留局部负更新并控制 far-field gradients；hard、global、linear 为对照或极限。
4. **Empirical Validation**：在 Online、D4RL、Countdown 验证任务效果、外部有效性、best/terminal stability 和 continuous/categorical 差异。

### Figure 1：From Useful Repulsion to Off-Policy Divergence

唯一默认双栏主图，横向三个 panel：

- Positive-only performance limit；
- controlled repulsion and finite better equilibrium；
- persistent far-field repulsion, divergence, and DRPO recovery。

不得绘制未经验证的动态 raw `p_t/q_t`；只画 fixed negative signal → increasing distance → stronger raw negative gradient → drift/divergence。
