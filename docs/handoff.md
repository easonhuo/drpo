...
- C-U1 E1/E2：现有正式状态保留。`C-U1-E3-ADAM-RERUN` 与 `C-U1-E4-ADAM-RERUN` 已获用户批准但仍须先提交本次代码；新 commit 前禁止启动。E3 Adam 交付前禁止 E4 Adam。
...
> **v29 覆盖说明：** 本节数值与 SGD 配置保留作历史 provenance。当前论文正式执行改为 `C-U1-E3-ADAM-RERUN`，状态“尚未运行”；旧结果不得用来声称 Adam 版已完成。
