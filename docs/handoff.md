...
> **E8 Reciprocal 400-cell 合并响应证据（2026-09-21）：** 四轮 `EXT-C-E8-MULTITASK-RECIPROCAL-MATRIX-01`、`EXT-C-E8-MULTITASK-RECIPROCAL-RIGHTTAIL-01`、`EXT-C-E8-MULTITASK-RECIPROCAL-RIGHTTAIL-CLOSURE-01`、`EXT-C-E8-MULTITASK-RECIPROCAL-LINEAR-GRAPHCOLOR-CLOSURE-01` 分别贡献 `128 + 104 + 108 + 60 = 400` cells。合并 compact evidence 含恰好 seeds `4000/5000`、无重复 `(task, method, lambda, seed)`、四个源包全部 cell complete、NaN/Inf 数值失败为 0。Reciprocal Linear 与 Reciprocal Quadratic 必须继续分开报告；新增右尾与 Graph Color 低区间加密点已基本闭合本轮描述性 response-shape follow-up。峰值描述不得只取孤立 raw maximum，优先采用局部多点平滑以避免 Graph Color / Maze 式尖峰主导解释。本证据仍只属于 finite-horizon / pilot response-curve evidence，不支持 convergence、steady-state、significance 或 universal method ranking。仓库 compact evidence 路径：CSV `experiments/results/e8_reciprocal_response_20260921/RECIPROCAL_400_CURVE_POINTS.csv`；图 `experiments/results/e8_reciprocal_response_20260921/reciprocal_merged_400_9grid.svg`；画图代码 `experiments/results/e8_reciprocal_response_20260921/PLOT_CODE.md`；摘要 `experiments/results/e8_reciprocal_response_20260921/RESULT_SUMMARY.md`。四个 raw ZIP 仍是独立 provenance artifact，继续归档到 `easonhuo/drpo-results`，compact evidence 不替代 raw archive。
