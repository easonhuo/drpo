...
Implementation base before this protocol: `5260ca58ecb67509838a1e5480f6dcaafdb7a773`.
...
Complete the high-lambda response tails for the eight non-Countdown tasks while preserving the historical 208-cell results for later concatenation. Countdown contributes zero new scientific cells. The successor scientific interface is lambda-only: manuscript scale `c = 1`, no rho-derived transport, and the same canonical paper trainer/taper/loss/negative-consumer/optimizer/data/evaluation path as the source COLDSTART experiment.
...
Countdown retains the six historical paper lambda sentinels in configuration only as an engineering-liveness anchor for the unchanged canonical runtime. `countdown_seed_offsets` is empty, so those sentinels create no scientific Countdown cells.
...
The successor YAML configuration is the sole runtime authority for the new lambda grid, seed offsets, expected cell count, and expected wave count. Those successor scientific values must not be duplicated as experiment-specific constants in Python.

Python may only provide minimal profile recognition, validation and expansion of the configuration. In particular, the implementation must not hard-code `8`, `25`, `199`, `8000`, `9000`, or any successor grid hash/value as lambda-completion-specific Python logic. The historical COLDSTART validation remains unchanged except where a shared generic helper is unavoidably used.

Successor Exp cells must be instantiated with `rho=None` and `lambda_value=<configured lambda>`. The historical optional rho field remains only for backward compatibility with historical experiment provenance; the successor configuration and scientific coefficient path do not expose rho.
...
Before any GPU launch, the implementation must prove all of the following:

1. Expanding the frozen successor YAML produces exactly the YAML-declared unique cell count and task/seed geometry.
2. Every successor Exp cell has `rho=None` and carries its configured lambda directly.
3. Every frozen successor lambda passes the exact historical-wrapper-vs-lambda-only taper/loss/gradient equivalence regression.
4. Canonical paper trainer/taper/loss/config source identities remain unchanged.
5. The old 208-cell COLDSTART config still validates and still produces exactly the historical 208 cells and 13 full 16-cell waves.
6. No successor scientific hyperparameter is hard-coded into Python.
