...
The successor YAML configuration is the sole authority for the new lambda grid, seed offsets, expected cell count, and expected wave count. Those successor scientific values must not be duplicated as experiment-specific constants in Python.

Python may only provide generic validation and expansion of the configuration. In particular, the implementation must not hard-code `8`, `25`, `199`, `8000`, `9000`, or any successor grid hash/value as lambda-completion-specific Python logic. The historical COLDSTART validation remains intact for the historical experiment ID.
...
3. Every frozen successor lambda passes the exact historical-vs-lambda-only taper/loss/gradient equivalence regression.
