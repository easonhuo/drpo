...
The reviewed pilot launch below is fresh-machine safe at the repository/data/model level and pins the canonical offline execution branch `dev/e8-multitask-exp-coldstart-01`. It uses a dedicated source checkout under `$HOME`, clones that branch when absent, otherwise requires the dedicated checkout to be clean and fast-forwards it only. Exact equality with `origin/dev/e8-multitask-exp-coldstart-01` is checked before the full bootstrap, so an unrelated `main` checkout, detached HEAD, or another project working tree cannot silently become the experiment source. The host base environment must already provide `bash`, `git`, Python 3.10+ with `pip` and `venv`, NVIDIA drivers, and a CUDA-compatible PyTorch build for which `torch.cuda.is_available()` is true; network access is also required. The bootstrap performs the remaining dependency installation, model/data acquisition, GPU-count/memory, disk, source-lock, liveness, scheduler, recovery, and packaging gates.

```bash
set -euo pipefail
REPO_URL="https://github.com/easonhuo/drpo.git"
BRANCH="dev/e8-multitask-exp-coldstart-01"
