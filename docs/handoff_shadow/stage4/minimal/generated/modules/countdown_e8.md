...
- Source hash: `35dacaa216387ecfbcd4d77a9ca1aca5c5a43dffa5c54073779ab9ec455c021f`
...
## Source 12: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-multitask-baseline-matrix-formal-closure-2026-09-18

### Delta block `section_end:e8-multitask-baseline-matrix-formal-closure-2026-09-18`

- **E8 176-cell baseline-matrix formal protocol authority alignment (`EXT-C-E8-MULTITASK-BASELINE-MATRIX-01`)：** 现有 `configs/e8_multitask_baseline_matrix_formal.yaml` 的冻结协议已通过 schema-v3 authority 登记到 registry / handoff；科学状态仍为 **not_run**，本闭环没有启动训练。正式矩阵只新增八个 P0 transfer tasks 的 cells，Countdown 只复用 predecessor evidence、不重跑；八任务 × paired seeds `4000/5000` × 每 task-seed 的 AsymRE 5 点、Joint Fitted-Reference beta-TOPR 3 点、canonical DPO 3 点，共 `176` cells。训练 horizon 固定 `1200` updates，保留 seed-4000 → seed-5000 barrier；DPO 使用 `base_model_fresh_lora` 与 exact frozen initial reference。该实验职责仅为 finite-horizon external-validity baseline comparison，不支持 convergence、significance 或 universal method ranking；方法比较必须经过 terminal audit，task performance、valid/structure diagnostic 与 NaN/Inf numerical failure 继续分开报告。工程重构 PR #362 / #370 已分别以 `3beb6500f447ef5424983f247bee2526d737e7d7` / `4166db1fc1e0af8d5023362e57d7b309be7b35ec` 合入 main，未改变上述科学协议；对应 scope 文档中 Draft/unmerged 文字保留为 pre-merge 历史快照，不做破坏性改写。registry 的 ready/active bookkeeping 只记录当前 implemented canonical execution route，不构成科学运行已经开始或完成的声明，也不替代 exact-commit、provenance、recovery、terminal-audit 与 artifact 要求。

## Source 13: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-multitask-coldstart-result-closure-2026-08-25
...
## Source 14: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-multitask-global-alpha-sweep-01-registration-2026-09-24

### Delta block `section_end:e8-multitask-global-alpha-sweep-01-registration-2026-09-24`

- **E8 Global-\(\alpha\) 128-cell transfer sweep protocol registration（`EXT-C-E8-MULTITASK-GLOBAL-ALPHA-SWEEP-01`；2026-09-24）：** 科学状态保持 **not_run**。正式矩阵只覆盖八个 P0 transfer tasks：`word_sorting`、`spiral_matrix`、`mini_sudoku`、`maze`、`word_ladder`、`knights_knaves`、`graph_color`、`wikisql`；Countdown 继续复用既有结果且不新增 cell，Positive-only（\(\alpha=0\)）也不重跑。Global-\(\alpha\) 固定使用旧 canonical negative kernel 的 `c=0` 常数权重语义，\(\alpha\in\{1/128,1/64,1/32,1/16,1/8,1/4,1/2,1\}\)，paired seeds 固定为 `4000/5000`，共 `8×8×2=128` cells；fresh LoRA、数据/bank、optimizer、1200-update horizon、evaluation cadence、task-local runtime、8 GPU × 2 slots 与现有 E8 transfer contract 保持一致，test partition 禁止用于本 sweep。该实验只提供 finite-horizon external-validity response evidence；不得据此宣称 convergence、steady state、significance 或 universal method ranking，任务性能、valid/structure 诊断与 NaN/Inf 数值失败继续分开报告。

## Source 15: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-onpolicy-capacity-diag-0p5b-gate
...
## Source 16: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-onpolicy-unpolished-0p5b-gate
...
## Source 17: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-oracle-offline-bank-v2-0p5b-gate
...
## Source 18: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-oracle-offline-v2-init-matrix-pilot-result
...
## Source 19: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-paper-aligned-lambda-round1-target-driven-registration
...
## Source 20: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-reciprocal-400-compact-evidence-2026-09-21

### Delta block `section_end:e8-reciprocal-400-compact-evidence-2026-09-21`

> **E8 Reciprocal 400-cell 合并响应证据（2026-09-21；2026-09-22 按 Figure 6 与 Table 1 资产规范对齐）：** 四轮 `EXT-C-E8-MULTITASK-RECIPROCAL-MATRIX-01`、`EXT-C-E8-MULTITASK-RECIPROCAL-RIGHTTAIL-01`、`EXT-C-E8-MULTITASK-RECIPROCAL-RIGHTTAIL-CLOSURE-01`、`EXT-C-E8-MULTITASK-RECIPROCAL-LINEAR-GRAPHCOLOR-CLOSURE-01` 分别贡献 `128 + 104 + 108 + 60 = 400` cells。合并 compact evidence 含恰好 seeds `4000/5000`、无重复 `(task, method, lambda, seed)`；合并图对应 `200` 个 two-seed means。Reciprocal Linear 与 Reciprocal Quadratic 必须继续分开报告；新增右尾与 Graph Color 低区间加密点已基本闭合本轮描述性 response-shape follow-up。峰值描述不得只取孤立 raw maximum，优先采用局部多点平滑以避免 Graph Color / Maze 式尖峰主导解释。本证据仍只属于 finite-horizon / pilot response-curve evidence，不支持 convergence、steady-state、significance 或 universal method ranking。资产现与 Figure 6 统一：canonical CSV `experiments/results/e8_reciprocal_response_20260921/RECIPROCAL_400_CURVE_POINTS.csv`；机器定位 `experiments/results/e8_reciprocal_response_20260921/RESULT_LOCATOR.json`；plot-ready mirror `paper/iclr2027/figures/data/e8_reciprocal_response_20260921/RECIPROCAL_400_CURVE_POINTS.csv`；画图入口 `scripts/figures/plot_e8_multitask_exp_response_curves.py --reciprocal`；checked-in working SVG `paper/iclr2027/figures/fig_e8_reciprocal_400_response_eight_panel.svg`。Table 1 的 structured-generation 行改为 `SG-9`，把 Countdown 与八个 transfer tasks 做未加权宏平均；派生数据位于 `experiments/results/e8_reciprocal_response_20260921/TABLE1_SG9_TAPER_AGGREGATE.csv`。当前 SG-9 显示值为 Positive-only `67.9`、Global-α `--`、Reciprocal-Linear `68.1`、Reciprocal-Quadratic `68.2`、DRPO/Exp `69.2`；Global-α 留空是因为八个 transfer tasks 的 protocol-matched Global-α sweep 尚未运行，不表示失败或 0。Countdown 不属于这 400-cell reciprocal sweeps，而是作为既有 Countdown 结果被并入 SG-9 aggregate。四个 raw ZIP 仍是独立 provenance artifact，继续归档到 `easonhuo/drpo-results`，compact evidence 不替代 raw archive。

## Source 21: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-reciprocal-joint-pilot-closure-2026-07-20
...
## Source 22: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-v2-active-taper-sweep-ready
...
## Source 23: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:e8-v2-global-low-scale-milestone-pilot
...
## Source 24: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v52-countdown-current-gate-override
...
## Source 25: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v57-countdown-offline-bank-current-gate
...
## Source 26: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v59-countdown-offline-bank-tuning-current-gate
...
## Source 27: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v62-countdown-online-offpolicy-current-gate
...
## Source 28: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v73-e8-taper-current-gate
...
## Source 29: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v75-e8-taper-diagnostic-bugfix
...
## Source 30: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v79-e8-active-tail-current-gate
...
## Source 31: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v52-e8-route-override
...
## Source 32: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v57-e8-offline-online-route
...
## Source 33: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v75-countdown-status-note
...
## Source 34: docs/handoff.md: HANDOFF-DELTA-BLOCK section_end:v52-execution-order-override
...
## Source 35: experiments/registry.yaml: experiments[EXT-C-E8-V4, EXT-C-E8-V4.1, EXT-C-E8-V4.2, EXT-C-E8-V4.3, EXT-C-E8-V4.4-OFFLINE-BANK, EXT-C-E8-V4.5-OFFLINE-BANK-TUNING, EXT-C-E8-V4.6-ONLINE-OFFPOLICY-REPLAY, EXT-C-E8-TAPER-0.5B-01, EXT-C-E8-SCALE-01]
