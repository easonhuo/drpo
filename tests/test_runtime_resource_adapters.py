...
def test_e7_selects_from_probe_then_reuses_exact_cache(
    ...
    def fake_measure(*_args, **_kwargs) -> CommandProbeResult:
        calls["count"] += 1
    ...
    monkeypatch.setattr(adapters, "measure_command_peak_memory", fake_measure)
    kwargs = dict(
    ...
    second = adapters.select_e7_runtime(**kwargs)
...
def test_cached_e7_selection_is_rejected_on_machine_static_change(tmp_path: Path) -> None:
    ...
    machine = snapshot(cpus=16, available_gib=64)
    path.write_text(
        yaml.safe_dump(
            {
                "adapter_id": adapters.E7_ADAPTER_ID,
                "resource_fingerprint_sha256": canonical_json_sha256(fingerprint),
                "machine_static_sha256": canonical_json_sha256(machine.static_identity()),
                "selection": {"selected_workers": 8},
                "probe": {"peak_rss_bytes": 1 * GIB},
            }
        ),
        encoding="utf-8",
    )
    # YAML is not JSON, so a damaged cache is safely treated as a miss.
    assert (
...
def test_e8_selection_is_limited_by_host_ram_and_vram(
    ...
    # floor(13 * .9 / 4) = 2 host-memory-safe GPU workers.
