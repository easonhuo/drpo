"""Automatic CPU/RAM selection for the E7 squared-EXP high-c boundary pilot."""
...
from drpo import e7_sqexp_actor_decision_runtime_autotune as predecessor
from drpo import e7_sqexp_highc_boundary as pilot


ADAPTER_ID = "e7_sqexp_highc_boundary_cpu_v1"
REPRESENTATIVE_DATASET = "walker2d-medium-v2"
REPRESENTATIVE_W0 = 1.0
REPRESENTATIVE_COEFFICIENT = 256.0
REPRESENTATIVE_ACTOR_MODE = "ppo_clip_kl_k4"
REPRESENTATIVE_CONTROL_ID = "squared_c256"
...
def _representative(branches: list[Any]) -> Any:
    matches = [
        ...
        and branch.template_values.get("control_id") == REPRESENTATIVE_CONTROL_ID
        and math.isclose(
    ...
    if len(matches) != 1:
        raise predecessor.legacy.RuntimeResourceError(
            "expected one representative squared-c256 PPO-K4-KL branch, "
            f"found {len(matches)}"
        )
    return matches[0]
...
def resource_fingerprint(
    *,
    ...
    source_paths = (
        ...
        "src/drpo/e7_sqexp_actor_decision.py",
        "src/drpo/e7_sqexp_actor_decision_bootstrap.py",
        "src/drpo/e7_sqexp_actor_decision_aggregate.py",
        "src/drpo/e7_sqexp_actor_decision_runtime_autotune.py",
        "src/drpo/e7_sqexp_highc_boundary.py",
        "src/drpo/e7_sqexp_highc_boundary_bootstrap.py",
        "src/drpo/e7_sqexp_highc_boundary_aggregate.py",
        "src/drpo/e7_sqexp_highc_boundary_runtime_autotune.py",
    ...
    return {
        ...
        "hard_fields": {
            "contract_sha256": predecessor.legacy._file_sha256(  # noqa: SLF001
                contract_path
            ),
            "run_spec_sha256": predecessor.legacy._file_sha256(  # noqa: SLF001
                run_spec_path
            ),
            "grid_sha256": predecessor.legacy._file_sha256(grid_path),  # noqa: SLF001
            "source_sha256": {
                path: predecessor.legacy._file_sha256(repo / path)  # noqa: SLF001
                for path in source_paths
            },
            "predecessor_implementation_commit": (
                pilot.PREDECESSOR_IMPLEMENTATION_COMMIT
            ),
            ...
            "representative_workload": {
                ...
                "control_id": REPRESENTATIVE_CONTROL_ID,
...
def _installed_predecessor_adapter() -> Iterator[None]:
    previous = (
        predecessor.pilot,
        predecessor.ADAPTER_ID,
        predecessor.REPRESENTATIVE_DATASET,
        predecessor.REPRESENTATIVE_W0,
        predecessor.REPRESENTATIVE_COEFFICIENT,
        predecessor.REPRESENTATIVE_ACTOR_MODE,
        predecessor.REPRESENTATIVE_CONTROL_ID,
        predecessor.PROXY,
        predecessor._representative,  # noqa: SLF001
        predecessor.resource_fingerprint,
    )
    predecessor.pilot = pilot
    predecessor.ADAPTER_ID = ADAPTER_ID
    predecessor.REPRESENTATIVE_DATASET = REPRESENTATIVE_DATASET
    predecessor.REPRESENTATIVE_W0 = REPRESENTATIVE_W0
    predecessor.REPRESENTATIVE_COEFFICIENT = REPRESENTATIVE_COEFFICIENT
    predecessor.REPRESENTATIVE_ACTOR_MODE = REPRESENTATIVE_ACTOR_MODE
    predecessor.REPRESENTATIVE_CONTROL_ID = REPRESENTATIVE_CONTROL_ID
    predecessor.PROXY = PROXY
    predecessor._representative = _representative  # noqa: SLF001
    predecessor.resource_fingerprint = resource_fingerprint
    try:
        yield
    finally:
        (
...
def select_runtime(**kwargs: Any) -> dict[str, Any]:
    with _installed_predecessor_adapter():
        return predecessor.select_runtime(**kwargs)
