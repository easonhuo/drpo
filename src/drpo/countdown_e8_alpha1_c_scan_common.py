#!/usr/bin/env python3
"""E8 V2 alpha=1 c-only scan development pilot.
...
def validate_grid_config(config: Mapping[str, Any]) -> None:
    if config.get("experiment_id") != EXPERIMENT_ID:
        raise ValueError("Alpha=1 c-only scan experiment_id mismatch")
    if config.get("result_status") != "pilot":
        raise ValueError("Continuous EXP grid must remain a pilot")
    if config.get("registration_state") not in ALLOWED_REGISTRATION_STATES:
        raise ValueError(
            "registration_state must be dev_code_first_unregistered or registered_pilot"
        )
    ...
    if configured_points != PARAMETER_POINTS:
        raise ValueError(f"parameter points changed: {configured_points}")
    ...
    if int(sweep.get("unique_parameter_points", -1)) != EXPECTED_POINTS:
        raise ValueError("The pilot requires 8 unique parameter points")
    if int(sweep.get("cells", -1)) != EXPECTED_CELLS:
        raise ValueError("The pilot requires 32 cells")
    if sweep.get("cartesian_product") is not False:
        raise ValueError("The c-only scan must remain an explicit non-Cartesian point list")
    ...
    if int(execution.get("parallel_cells_per_gpu", -1)) != 2:
        raise ValueError("The pilot requires two runtime cells per selected GPU")
...
def parameter_points(config: Mapping[str, Any]) -> tuple[tuple[float, float], ...]:
    ...
    if points != PARAMETER_POINTS or len(set(points)) != EXPECTED_POINTS:
        raise AssertionError("Alpha=1 c-only scan must produce 8 unique points")
    return points
...
def build_cells(config: Mapping[str, Any]) -> tuple[Cell, ...]:
    ...
    if (
        ...
        raise AssertionError("Alpha=1 c-only scan must produce 32 unique cells")
    return cells
...
def mean_unique_negative_term(
    ...
    """Average by K_p per prompt, then average prompts; never by weight sum."""
    ...
    *,
    ...
    source_paths = {
        ...
        "trainer": Path(__file__).resolve().with_name(
            "countdown_e8_alpha1_c_scan_trainer.py"
        ),
        "runtime": Path(__file__).resolve().with_name(
            "countdown_e8_alpha1_c_scan_runtime.py"
        ),
        "auto_launcher": repo
        / "scripts"
    ...
    if missing_sources:
        raise RuntimeError(
            "Alpha=1 c-only scan identity is missing protected sources: "
