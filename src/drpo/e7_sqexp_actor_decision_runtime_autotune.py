"""Automatic CPU/RAM selection for the E7 actor/high-c decision pilot."""
...
from drpo import e7_sqexp_actor_decision as pilot


ADAPTER_ID = "e7_sqexp_actor_decision_cpu_v1"
REPRESENTATIVE_DATASET = "walker2d-medium-v2"
REPRESENTATIVE_W0 = 1.0
REPRESENTATIVE_COEFFICIENT = 64.0
REPRESENTATIVE_ACTOR_MODE = "ppo_clip_kl_k4"
REPRESENTATIVE_CONTROL_ID = "squared_c64"
...
def _representative(branches: list[Any]) -> Any:
    ...
    if len(matches) != 1:
        raise legacy.RuntimeResourceError(
            "expected one representative squared-c64 PPO-K4-KL branch, "
            f"found {len(matches)}"
        )
    return matches[0]
    ...
    *,
    ...
    return {
        ...
        "ignored_scientific_coordinates": [
            ...
            "linear_and_squared_control_grid",
            "training_horizon",
        ],
