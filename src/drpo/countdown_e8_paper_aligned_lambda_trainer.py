#!/usr/bin/env python3
"""Training core for the E8 paper-aligned lambda code-first pilot."""
...
def train_cell(
    *,
    ...
    if calibration.get("experiment_id") != EXPERIMENT_ID:
        raise ValueError("Calibration experiment_id mismatch")
    ...
    evaluation_seed = seed + int(grid_config["evaluation"]["paired_evaluation_seed_offset"])
    arena.seed_all(seed)
    ...
    dataset = ContinuousUniqueBankDataset(train_rows, tokenizer, int(model_cfg["max_length"]))
    ...
    for update_step in range(1, steps + 1):
        ...
        diagnostic_keys = (
            ...
            "negative_surprisal_p10",
            "negative_surprisal_p50",
            "negative_surprisal_p90",
            "normalized_excess_mean",
            "normalized_excess_p10",
            "normalized_excess_p50",
            "normalized_excess_p90",
            "active_tail_fraction",
            "weight_mean",
            "weight_p10",
            "weight_p50",
            "weight_p90",
        ...
        for _ in range(int(train_cfg["gradient_accumulation"])):
            ...
            if cell.alpha == 0.0:
                weighted_negative_lp = torch.zeros((), device=device, dtype=positive_lp.dtype)
                diagnostics = {
                    "negative_surprisal_mean": None,
                    "negative_surprisal_p10": None,
                    "negative_surprisal_p50": None,
                    "negative_surprisal_p90": None,
                    "normalized_excess_mean": None,
                    "normalized_excess_p10": None,
                    "normalized_excess_p50": None,
                    "normalized_excess_p90": None,
                    "active_tail_fraction": None,
                    "weight_mean": 0.0,
                    "weight_p10": 0.0,
                    "weight_p50": 0.0,
                    "weight_p90": 0.0,
                    ...
                    "duplicates_removed_mean": float(
                        (packed["raw_bank_counts"] - packed["unique_counts"]).float().mean()
                    ),
                }
            else:
                ...
                weights = paper_aligned_lambda_weights(
                    bank_stats["seq_lp"],
                    alpha=cell.alpha,
            ...
            accum["loss"] = float(accum["loss"] or 0.0) + float(raw_loss.detach()) / divisor
            accum["positive_lp"] = (
                float(accum["positive_lp"] or 0.0) + float(positive_lp.detach()) / divisor
            )
            accum["weighted_negative_lp"] = (
                float(accum["weighted_negative_lp"] or 0.0)
        ...
        if not bool(torch.isfinite(raw_grad_norm)):
            ...
            stop_reason = numerical_failure
            break
        sample_update_norm = update_step % int(train_cfg["log_every"]) == 0 or update_step == steps
        before = (
        ...
        if not arena.optimizer_step_with_last_finite_guard(optimizer, trainable):
            ...
            stop_reason = numerical_failure
            break
        update_norm = _parameter_update_norm(before, trainable) if sample_update_norm else None
        scheduler.step()
        ...
        if update_step % eval_every == 0 or update_step == steps:
            ...
            if value > best_pass8:
                ...
                checkpoint_records = [
                    record for record in checkpoint_records if record["kind"] != "best_pass8"
                ]
                checkpoint_records.append(
    ...
    def late_mean(key: str, window: int = 3) -> float | None:
        values = [float(row[key]) for row in metric_rows if row.get(key) is not None]
        return float(sum(values[-window:]) / len(values[-window:])) if values else None

    late_window_metrics = {
        "window_evaluations": 3,
        "val_greedy": late_mean("val_greedy"),
        "val_pass_at_8": late_mean("val_pass_at_8"),
        "val_pass_at_64": late_mean("val_pass_at_64"),
        "val_valid_rate": late_mean("val_valid_rate"),
    }
    manifest = {
        "schema_version": 1,
        ...
        "alpha": cell.alpha,
        "lambda": cell.lambda_value,
        "weight_formula": "alpha*exp(-lambda*relu((D-tau)/scale_c))",
        "D_definition": "negative_mean_completion_token_log_probability_with_eos",
        "tau": tau,
        "scale_c": scale_c,
        "effective_beta": (cell.lambda_value / scale_c if cell.alpha > 0 else 0.0),
        "extra_square_used": False,
        ...
        "bank_audit": {
            ...
            "rows_with_duplicates_removed": sum(
                raw > unique for raw, unique in zip(raw_counts, unique_counts, strict=True)
            ),
        },
